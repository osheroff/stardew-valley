...
namespace StardewValley.Tools
{
	public class FishingRod : Tool
	{
		...
		public override void DoFunction(GameLocation location, int x, int y, int power, Farmer who)
		{
			...
			else
			{
				...
				if (lastUser.FacingDirection == 1 || lastUser.FacingDirection == 3)
				{
					...
					t *= 1.2f;
					animations.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(170, 1903, 7, 8), t, 1, 0, bobber + new Vector2(-32f, -48f), flicker: false, flipped: false, (float)who.getStandingY() / 10000f, 0f, Color.White, 4f, 0f, 0f, (float)Game1.random.Next(-20, 20) / 100f)
					{
				...
				else
				{
					...
					animations.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(170, 1903, 7, 8), t2, 1, 0, bobber + new Vector2(-32f, -48f), flicker: false, flipped: false, bobber.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, (float)Game1.random.Next(-20, 20) / 100f)
					{
		...
		public void startMinigameEndFunction(int extra)
		{
			...
			Game1.activeClickableMenu = new BobberBar(extra, fishSize, treasure, (attachments[1] != null) ? attachments[1].ParentSheetIndex : (-1));
		}
		...
		private void doPullFishFromWater(BinaryReader argReader)
		{
			...
			Rectangle sprite_rect = default(Rectangle);
			if (itemCategory == "Object")
			{
				sprite_sheet_name = "Maps\\springobjects";
				sprite_rect = Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, whichFish, 16, 16);
			}
			else
			{
			...
			if (lastUser.FacingDirection == 1 || lastUser.FacingDirection == 3)
			{
				float distance = Vector2.Distance(bobber, lastUser.Position);
				float gravity = 0.001f;
				float height = 128f - (lastUser.Position.Y - bobber.Y + 10f);
				...
				animations.Add(new TemporaryAnimatedSprite(sprite_sheet_name, sprite_rect, t, 1, 0, bobber, flicker: false, flipped: false, bobber.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
					motion = new Vector2((float)((lastUser.FacingDirection != 3) ? 1 : (-1)) * (0f - xVelocity), 0f - yVelocity),
					acceleration = new Vector2(0f, gravity),
					timeBasedMotion = true,
				...
				if (caughtDoubleFish)
				{
					distance = Vector2.Distance(bobber, lastUser.Position);
					gravity = 0.0008f;
					height = 128f - (lastUser.Position.Y - bobber.Y + 10f);
			...
			{
				float distance2 = bobber.Y - (float)(lastUser.getStandingY() - 64);
				...
				if (t != 0f)
				{
					xVelocity2 = (lastUser.Position.X - bobber.X) / t;
				}
				...
				if (caughtDoubleFish)
				{
					distance2 = bobber.Y - (float)(lastUser.getStandingY() - 64);
			...
			{
			case 0:
				lastUser.FarmerSprite.animateBackwardsOnce(299, t);
				break;
			case 1:
				lastUser.FarmerSprite.animateBackwardsOnce(300, t);
				break;
			case 2:
				lastUser.FarmerSprite.animateBackwardsOnce(301, t);
				break;
			case 3:
				lastUser.FarmerSprite.animateBackwardsOnce(302, t);
				break;
			}
		}
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (!bobber.Value.Equals(Vector2.Zero) && (isFishing || pullingOutOfWater || castedButBobberStillInAir) && lastUser.FarmerSprite.CurrentFrame != 57 && (lastUser.FacingDirection != 0 || !pullingOutOfWater || whichFish == -1))
			{
				...
				if (castedButBobberStillInAir)
				{
					...
					{
					case 2:
						lastPosition = lastUser.FarmerSprite.currentAnimationIndex switch
						{
				...
				{
					lastPosition = lastUser.FacingDirection switch
					{
			...
			if (castingChosenCountdown > 0f)
			{
				...
				if (castingChosenCountdown <= 0f && who.CurrentTool != null)
				{
					...
					if (who.FacingDirection == 1 || who.FacingDirection == 3)
					{
						...
						animations.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(170, 1903, 7, 8), t2, 1, 0, who.Position + new Vector2(0f, -96f), flicker: false, flipped: false, (float)who.getStandingY() / 10000f, 0f, Color.White, 4f, 0f, 0f, (float)Game1.random.Next(-20, 20) / 100f)
						{
