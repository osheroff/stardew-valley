...
namespace StardewValley.Tools
{
	public class MeleeWeapon : Tool
	{
		...
		public int getNumberOfDescriptionCategories()
		{
			...
			if ((double)effectiveCritChance / 0.02 >= 1.1000000238418579)
			{
				number++;
			}
		...
		public int getItemLevel()
		{
			...
			weaponPoints += (float)(int)((double)((int)addedPrecision / 2 + (int)addedDefense) + ((double)(float)critChance - 0.02) * 200.0 + (double)(((float)critMultiplier - 3f) * 6f));
		...
		public void DoDamage(GameLocation location, int x, int y, int facingDirection, int power, Farmer who)
		{
			...
			if (location.damageMonster(areaOfEffect, (int)((float)(int)minDamage * (1f + who.attackIncreaseModifier)), (int)((float)(int)maxDamage * (1f + who.attackIncreaseModifier)), isBomb: false, (float)knockback * (1f + who.knockbackModifier), (int)((float)(int)addedPrecision * (1f + who.weaponPrecisionModifier)), effectiveCritChance * (1f + who.critChanceModifier), (float)critMultiplier * (1f + who.critPowerModifier), (int)type != 1 || !isOnSpecial, lastUser) && (int)type == 2)
			{
		...
		public override void drawTooltip(SpriteBatch spriteBatch, ref int x, ref int y, SpriteFont font, float alpha, StringBuilder overrideText)
		{
			...
			if ((double)effectiveCritChance / 0.02 >= 1.1000000238418579)
			{
				...
				Utility.drawTextWithShadow(spriteBatch, Game1.content.LoadString("Strings\\UI:ItemHover_CritChanceBonus", (int)Math.Round((double)(effectiveCritChance - 0.001f) / 0.02)), font, new Vector2(x + 16 + 52, y + 16 + 12), c4 * 0.9f * alpha);
				y += (int)Math.Max(font.MeasureString("TT").Y, 48f);
			}
			if ((double)((float)critMultiplier - 3f) / 0.02 >= 1.0)
			{
			...
			if (enchantments.Count > 0 && enchantments[enchantments.Count - 1] is DiamondEnchantment)
			{
				...
				string random_forge_string = "";
				random_forge_string = ((random_forges != 1) ? Game1.content.LoadString("Strings\\UI:ItemHover_DiamondForge_Plural", random_forges) : Game1.content.LoadString("Strings\\UI:ItemHover_DiamondForge_Singular", random_forges));
