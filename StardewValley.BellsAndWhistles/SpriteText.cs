	...
	public class SpriteText
	{
		...
		public static int getWidthOfString(string s, int widthConstraint = 999999)
		{
			...
			for (int i = 0; i < s.Length; i++)
			{
				...
				{
					if (_characterMap.TryGetValue(s[i], out var c))
					{
						width += c.XAdvance;
					}
		...
		public static int getHeightOfString(string s, int widthConstraint = 999999)
		{
			...
			if (!LocalizedContentManager.CurrentLanguageLatin && !forceEnglishFont)
			{
				for (int j = 0; j < s.Length; j++)
				{
					...
					if (_characterMap.TryGetValue(s[j], out var c))
					{
		...
		public static int positionOfNextSpace(string s, int index, int currentXPosition, int accumulatedHorizontalSpaceBetweenCharacters)
		{
			...
			{
				if (_characterMap.TryGetValue(s[index], out var fc))
				{
			...
			{
				if (_characterMap.TryGetValue(s[index], out var fc3))
				{
			...
			for (int i = index; i < s.Length; i++)
			{
				if (!LocalizedContentManager.CurrentLanguageLatin)
				{
					...
					currentXPosition = ((!_characterMap.TryGetValue(s[i], out var fc2)) ? (currentXPosition + (int)((float)FontFile.Common.LineHeight * fontPixelZoom)) : (currentXPosition + (int)((float)fc2.XAdvance * fontPixelZoom)));
					continue;
				}
				if (s[i] == ' ' || s[i] == '^')
				{
