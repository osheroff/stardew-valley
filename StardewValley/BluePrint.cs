...
namespace StardewValley
{
	public class BluePrint
	{
		...
		public void drawDescription(SpriteBatch b, int x, int y, int width)
		{
			...
			int yPosition = (int)((float)y + Game1.smallFont.MeasureString(name).Y + Game1.smallFont.MeasureString(descriptionString).Y);
			foreach (KeyValuePair<int, int> kvp in itemsRequired)
			{
				b.Draw(Game1.objectSpriteSheet, new Vector2(x + 8, yPosition), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, kvp.Key, 16, 16), Color.White, 0f, new Vector2(6f, 3f), 2f, SpriteEffects.None, 0.999f);
				Color colorToDrawResource2 = (Game1.player.hasItemInInventory(kvp.Key, kvp.Value) ? Color.DarkGreen : Color.DarkRed);
				Utility.drawTinyDigits(kvp.Value, b, new Vector2((float)(x + 32) - Game1.tinyFont.MeasureString(string.Concat(kvp.Value)).X, (float)(yPosition + 32) - Game1.tinyFont.MeasureString(string.Concat(kvp.Value)).Y), 1f, 0.9f, Color.AntiqueWhite);
				b.DrawString(Game1.smallFont, Game1.objectInformation[kvp.Key].Split('/')[4], new Vector2(x + 32 + 16, yPosition), colorToDrawResource2);
				yPosition += (int)Game1.smallFont.MeasureString("P").Y;
			}
			if (moneyRequired > 0)
			{
				b.Draw(Game1.debrisSpriteSheet, new Vector2(x, yPosition), Game1.getSourceRectForStandardTileSheet(Game1.debrisSpriteSheet, 8), Color.White, 0f, new Vector2(24f, 11f), 0.5f, SpriteEffects.None, 0.999f);
				Color colorToDrawResource = ((Game1.player.Money >= moneyRequired) ? Color.DarkGreen : Color.DarkRed);
				b.DrawString(Game1.smallFont, Game1.content.LoadString("Strings\\StringsFromCSFiles:LoadGameMenu.cs.11020", moneyRequired), new Vector2(x + 16 + 8, yPosition), colorToDrawResource);
				yPosition += (int)Game1.smallFont.MeasureString(string.Concat(moneyRequired)).Y;
			}
		}
	}
}
