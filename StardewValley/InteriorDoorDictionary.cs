...
namespace StardewValley
{
	public class InteriorDoorDictionary : NetPointDictionary<bool, InteriorDoor>
	{
		...
		{
			if (!location.isOutdoors && location.Map.Properties.TryGetValue("Doors", out var doorsValue) && doorsValue != null)
			{
		...
		{
			if ((bool)location.isOutdoors || !location.Map.Properties.TryGetValue("Doors", out var doorsValue) || doorsValue == null)
			{
				return;
			}
			string[] doorsParams = doorsValue.ToString().Split(' ');
