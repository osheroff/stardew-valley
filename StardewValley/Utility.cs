...
namespace StardewValley
{
	public class Utility
	{
		...
		public static int getStartTimeOfFestival()
		{
			...
			{
				return Convert.ToInt32(Game1.temporaryContent.Load<Dictionary<string, string>>(string.Concat("Data\\Festivals\\" + Game1.currentSeason, Game1.dayOfMonth))["conditions"].Split('/')[1].Split(' ')[0]);
			}
			return -1;
		}
			...
			for (int i = 0; i < 10; i++)
			{
				...
				while (true)
				{
					...
					if (Game1.objectInformation.ContainsKey(index) && !isObjectOffLimitsForSale(index))
					{
						...
						if (split[3].Contains('-') && Convert.ToInt32(split[1]) > 0 && !split[3].Contains("-13") && !split[3].Equals("Quest") && !split[0].Equals("Weeds") && !split[3].Contains("Minerals") && !split[3].Contains("Arch") && addToStock(stock, stockIndices, new Object(index, 1), new int[2]
						{
		...
		private static Furniture getRandomFurniture(Random r, Dictionary<ISalable, int[]> stock, int lowerIndexBound = 0, int upperIndexBound = 1462)
		{
			...
			while (isFurnitureOffLimitsForSale(index) || !furnitureData.ContainsKey(index));
		...
		public static Keys mapGamePadButtonToKey(Buttons b)
		{
			return b switch
			{
				...
				Buttons.RightTrigger => Game1.options.getFirstKeyboardKeyFromInputButtonList(Game1.options.toolSwapButton), 
				Buttons.LeftTrigger => Game1.options.getFirstKeyboardKeyFromInputButtonList(Game1.options.toolSwapButton), 
