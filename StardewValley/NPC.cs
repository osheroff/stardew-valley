...
namespace StardewValley
{
	public class NPC : Character, IComparable
	{
		...
		{
			string textureName = character_name switch
			{
				"Old Mariner" => "Mariner", 
		...
		public int getGiftTasteForThisItem(Item item)
		{
			...
			if (item is Object)
			{
				...
				Game1.NPCGiftTastes.TryGetValue(base.Name, out var NPCLikes);
				...
				if (NPCLikes != null)
				{
					...
					{
						string[] splitItems = split[i + 1].Split(' ');
		...
		public bool CanRevisitLeoMemory(KeyValuePair<string, int>? event_data)
		{
			...
			foreach (string key in location_events.Keys)
			{
				if (key.Split('/')[0] == event_id.ToString())
				{
					...
					if (location != null && location.checkEventPrecondition(event_key) != -1)
					{
						return true;
					}
				}
		...
		public virtual void performTenMinuteUpdate(int timeOfDay, GameLocation l)
		{
			...
			{
				string[] split = Dialogue[string.Concat(l.name, "_Ambient")].Split('/');
		...
		public virtual void arriveAt(GameLocation l)
		{
			...
			{
				string[] split = Dialogue[string.Concat(l.name, "_Entry")].Split('/');
		...
		public string tryToGetMarriageSpecificDialogueElseReturnDefault(string dialogueKey, string defaultMessage = "")
		{
			...
			try
			{
				marriageDialogues = Game1.content.Load<Dictionary<string, string>>("Characters\\Dialogue\\MarriageDialogue" + GetDialogueSheetName());
			}
			catch (Exception)
			{
			}
		...
		private Stack<Dialogue> loadCurrentDialogue()
		{
			...
			{
				Dictionary<string, string> npcDispositions = Game1.content.Load<Dictionary<string, string>>("Data\\NPCDispositions");
				if (npcDispositions.TryGetValue(base.Name, out var disposition))
				{
					...
					if (relatives.Length > 1)
					{
						...
						if (npcGiftTastes.ContainsKey(relativeName))
						{
							...
							if (r.NextDouble() < 0.5)
							{
								...
								if (Game1.objectInformation.TryGetValue(item, out var itemDetails))
								{
		...
		public bool checkForNewCurrentDialogue(int heartLevel, bool noPreface = false)
		{
			...
			foreach (string s in Game1.player.activeDialogueEvents.Keys)
			{
				...
				eventMessageKey = s;
				if (!eventMessageKey.Equals("") && !Game1.player.mailReceived.Contains(base.Name + "_" + eventMessageKey))
				{
					CurrentDialogue.Clear();
					CurrentDialogue.Push(new Dialogue(Dialogue[eventMessageKey], this));
			...
			if (Dialogue.ContainsKey(string.Concat(preface, Game1.currentLocation.name, "_", Game1.shortDayNameFromDayOfSeason(Game1.dayOfMonth))))
			{
				CurrentDialogue.Push(new Dialogue(Dialogue[string.Concat(preface, Game1.currentLocation.name, "_", Game1.shortDayNameFromDayOfSeason(Game1.dayOfMonth))], this)
				{
					removeOnNextMove = true
				});
				return true;
			}
			if (heartLevel >= 10 && Dialogue.ContainsKey(string.Concat(preface, Game1.currentLocation.name, "10")))
			{
				CurrentDialogue.Push(new Dialogue(Dialogue[string.Concat(preface, Game1.currentLocation.name, "10")], this)
				{
					removeOnNextMove = true
				});
				return true;
			}
			if (heartLevel >= 8 && Dialogue.ContainsKey(string.Concat(preface, Game1.currentLocation.name, "8")))
			{
				CurrentDialogue.Push(new Dialogue(Dialogue[string.Concat(preface, Game1.currentLocation.name, "8")], this)
				{
					removeOnNextMove = true
				});
				return true;
			}
			if (heartLevel >= 6 && Dialogue.ContainsKey(string.Concat(preface, Game1.currentLocation.name, "6")))
			{
				CurrentDialogue.Push(new Dialogue(Dialogue[string.Concat(preface, Game1.currentLocation.name, "6")], this)
				{
					removeOnNextMove = true
				});
				return true;
			}
			if (heartLevel >= 4 && Dialogue.ContainsKey(string.Concat(preface, Game1.currentLocation.name, "4")))
			{
				CurrentDialogue.Push(new Dialogue(Dialogue[string.Concat(preface, Game1.currentLocation.name, "4")], this)
				{
					removeOnNextMove = true
				});
				return true;
			}
			if (heartLevel >= 2 && Dialogue.ContainsKey(string.Concat(preface, Game1.currentLocation.name, "2")))
			{
				CurrentDialogue.Push(new Dialogue(Dialogue[string.Concat(preface, Game1.currentLocation.name, "2")], this)
				{
					removeOnNextMove = true
				});
				return true;
		...
		public Dialogue tryToRetrieveDialogue(string preface, int heartLevel, string appendToEnd = "")
		{
			...
			if (Dialogue.ContainsKey(preface + day_name + appendToEnd))
			{
				...
				if (!Dialogue.ContainsKey(preface + day_name + "_" + year + appendToEnd))
				{
					return new Dialogue(Dialogue[string.Concat(preface + day_name, appendToEnd)], this);
				}
				return new Dialogue(Dialogue[preface + day_name + "_" + year + appendToEnd], this);
			}
			return null;
		...
		public void checkForMarriageDialogue(int timeOfDay, GameLocation location)
		{
			...
			case 1800:
				if (location is FarmHouse)
				{
					...
					suffix = ((which < 0) ? base.Name : string.Concat(which));
					currentMarriageDialogue.Clear();
		...
		public Dictionary<int, SchedulePathDescription> getSchedule(int dayOfMonth)
		{
			...
			int friendship = Utility.GetAllPlayerFriendshipLevel(this);
			if (friendship >= 0)
			{
				friendship /= 250;
			}
			while (friendship > 0)
			{
				if (hasMasterScheduleEntry(Game1.dayOfMonth + "_" + friendship))
				{
					return parseMasterSchedule(getMasterScheduleEntry(Game1.dayOfMonth + "_" + friendship));
				}
				friendship--;
			}
			...
			friendship = Utility.GetAllPlayerFriendshipLevel(this);
			if (friendship >= 0)
			{
				friendship /= 250;
			}
		...
		public void marriageDuties()
		{
			...
			else if (spouse.GetDaysMarried() >= 1 && r.NextDouble() < 0.045)
			{
				if (r.NextDouble() < 0.75)
				{
					...
					try
					{
						new_furniture = new Furniture(Utility.getRandomSingleTileFurniture(r), new Vector2(spot2.X, spot2.Y));
					}
					catch (Exception)
					{
						new_furniture = null;
					}
					if (new_furniture != null && spot2.X > 0 && farmHouse.isTileLocationOpen(new Location(spot2.X - 1, spot2.Y)))
					{
