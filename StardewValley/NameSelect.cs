	...
	public class NameSelect
	{
		...
		public static void draw()
		{
			...
			for (int j = 0; j < 9; j++)
			{
				if (name.Length > j)
				{
					Game1.spriteBatch.DrawString(Game1.dialogueFont, name[j].ToString() ?? "", new Vector2((float)(xLocation + 128 + titleMessageWidth) + Game1.dialogueFont.MeasureString(underline).X + (Game1.dialogueFont.MeasureString("_").X - Game1.dialogueFont.MeasureString(name[j].ToString() ?? "").X) / 2f - 2f, yLocation + 128 - 6), Game1.textColor);
				}
				underline += "_ ";
			}
			...
			for (int i = 0; i < 5; i++)
			{
				...
				for (int k = 0; k < 15; k++)
				{
					...
					Game1.spriteBatch.DrawString(Game1.dialogueFont, namingCharacters[i * 15 + k].ToString() ?? "", new Vector2(sectionBuffer + xLocation + 64 + widthBetweenCharacters * k, yLocation + 192 + heightBetweenCharacters * i), Game1.textColor);
					if (selection == i * 15 + k)
					{
						Game1.spriteBatch.Draw(Game1.objectSpriteSheet, new Vector2(sectionBuffer + xLocation + widthBetweenCharacters * k - 6, yLocation + 192 + heightBetweenCharacters * i - 8), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, 26), Color.White);
					}
				}
			}
			if (selection == -1)
