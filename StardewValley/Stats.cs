		...
		public void monsterKilled(string name)
		{
			if (specificMonstersKilled.ContainsKey(name))
			{
				if (AdventureGuild.willThisKillCompleteAMonsterSlayerQuest(name))
				{
					...
					if (Game1.content.Load<Dictionary<string, string>>("Data\\Monsters").TryGetValue(name, out localized_name))
					{
						string[] split = localized_name.Split('/');
						localized_name = ((split.Length <= 14) ? name : split[14]);
					}
					else
					{
						localized_name = name;
					...
					Game1.multiplayer.globalChatInfoMessage("MonsterSlayer" + Game1.random.Next(4), Game1.player.Name, localized_name);
