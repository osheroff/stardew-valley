...
namespace StardewValley
{
	...
	public class Game1 : InstanceGame
	{
		...
		public bool isRenderingScreenBuffer;
		...
		public string takeMapScreenshot(float? in_scale, string screenshot_name, Action onDone)
		{
			...
			do
			{
				...
				try
				{
					map_bitmap = new Bitmap(scaled_width, scaled_height);
				}
				catch (Exception e2)
				{
		...
		public virtual void SetWindowSize(int w, int h)
		{
			...
			try
			{
				int sw = Math.Min(4096, (int)Math.Ceiling((float)localMultiplayerWindow.Width * (1f / options.zoomLevel)));
				int sh = Math.Min(4096, (int)Math.Ceiling((float)localMultiplayerWindow.Height * (1f / options.zoomLevel)));
				...
				screen.Name = "Screen";
				int uw = Math.Min(4096, (int)Math.Ceiling((float)localMultiplayerWindow.Width / options.uiScale));
				int uh = Math.Min(4096, (int)Math.Ceiling((float)localMultiplayerWindow.Height / options.uiScale));
		...
		public static void updateViewportForScreenSizeChange(bool fullscreenChange, int width, int height)
		{
			...
			Microsoft.Xna.Framework.Point center = new Microsoft.Xna.Framework.Point(viewport.X + viewport.Width / 2, viewport.Y + viewport.Height / 2);
			...
			{
				center = new Microsoft.Xna.Framework.Point(viewport.X + viewport.Width / 2, viewport.Y + viewport.Height / 2);
				viewport = new xTile.Dimensions.Rectangle(center.X - width / 2, center.Y - height / 2, width, height);
		...
		protected override void Initialize()
		{
			...
			screen = new RenderTarget2D(graphics.GraphicsDevice, graphics.GraphicsDevice.Viewport.Width, graphics.GraphicsDevice.Viewport.Height, mipMap: false, SurfaceFormat.Color, DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
		...
		protected override void LoadContent()
		{
			...
			Microsoft.Xna.Framework.Color[] white = new Microsoft.Xna.Framework.Color[1]
			{
		...
		public static void performTenMinuteClockUpdate()
		{
			hooks.OnGame1_PerformTenMinuteClockUpdate(delegate
			{
				...
				if (weatherIcon == 1)
				{
					int num = Convert.ToInt32(temporaryContent.Load<Dictionary<string, string>>(string.Concat("Data\\Festivals\\" + currentSeason, dayOfMonth))["conditions"].Split('/')[1].Split(' ')[0]);
					if (whereIsTodaysFest == null)
					{
						whereIsTodaysFest = temporaryContent.Load<Dictionary<string, string>>(string.Concat("Data\\Festivals\\" + currentSeason, dayOfMonth))["conditions"].Split('/')[0];
					}
					if (timeOfDay == num)
					{
						string text = temporaryContent.Load<Dictionary<string, string>>(string.Concat("Data\\Festivals\\" + currentSeason, dayOfMonth))["conditions"].Split('/')[0];
						switch (text)
						{
						case "Forest":
						...
						showGlobalMessage(content.LoadString("Strings\\StringsFromCSFiles:Game1.cs.2640", temporaryContent.Load<Dictionary<string, string>>(string.Concat("Data\\Festivals\\" + currentSeason, dayOfMonth))["name"]) + text);
					}
				}
		...
		private static IEnumerator<int> _newDayAfterFade()
		{
			...
			for (int m = player.enchantments.Count - 1; m >= 0; m--)
			{
				player.enchantments[m].OnUnequip(player);
			}
			player.dayupdate();
			if (IsMasterGame)
			{
			...
			foreach (Item l in shippingBin)
			{
				player.displayedShippedItems.Add(l);
			}
		...
		{
			if (MasterPlayer.mailReceived.Contains("Farm_Eternal") || !MasterPlayer.hasCompletedCommunityCenter() || !player.team.farmPerfect.Value)
			{
				return;
			}
		...
		public static void warpFarmer(LocationRequest locationRequest, int tileX, int tileY, int facingDirectionAfterWarp)
		{
			...
			if (weatherIcon == 1 && whereIsTodaysFest != null && locationRequest.Name.Equals(whereIsTodaysFest) && !warpingForForcedRemoteEvent && timeOfDay <= Convert.ToInt32(temporaryContent.Load<Dictionary<string, string>>(string.Concat("Data\\Festivals\\" + currentSeason, dayOfMonth))["conditions"].Split('/')[1].Split(' ')[1]))
			{
				if (timeOfDay < Convert.ToInt32(temporaryContent.Load<Dictionary<string, string>>(string.Concat("Data\\Festivals\\" + currentSeason, dayOfMonth))["conditions"].Split('/')[1].Split(' ')[0]))
				{
		...
		public static void PushUIMode()
		{
			...
			if (game1.isDrawing)
			{
				if (game1.uiScreen != null && !game1.uiScreen.IsDisposed)
				{
					...
					graphics.GraphicsDevice.SetRenderTarget(game1.uiScreen);
				}
		...
		public static void PopUIMode()
		{
			...
			if (game1.isDrawing)
			{
				if (graphics.GraphicsDevice.GetRenderTargets().Length != 0 && graphics.GraphicsDevice.GetRenderTargets()[0].RenderTarget == game1.uiScreen)
				{
					...
					{
						graphics.GraphicsDevice.SetRenderTarget(nonUIRenderTarget);
					}
					else
					{
						graphics.GraphicsDevice.SetRenderTarget(null);
		...
		public bool parseDebugInput(string debugInput)
		{
			...
			try
			{
				...
				case "perfection":
				{
					...
					for (int i16 = player.craftingRecipes.Count() - 1; i16 >= 0; i16--)
					{
						player.craftingRecipes[player.craftingRecipes.Pairs.ElementAt(i16).Key] = 1;
					}
				...
				case "killnpc":
				{
					for (int i17 = locations.Count - 1; i17 >= 0; i17--)
					{
						for (int j7 = 0; j7 < locations[i17].characters.Count; j7++)
						{
							if (locations[i17].characters[j7].Name.Equals(debugSplit[1]))
							{
								locations[i17].characters.RemoveAt(j7);
								break;
							}
						}
					}
				...
				case "growcrops":
					foreach (KeyValuePair<Vector2, TerrainFeature> t5 in currentLocation.terrainFeatures.Pairs)
					{
						...
						for (int i18 = 0; i18 < Convert.ToInt32(debugSplit[1]); i18++)
						{
				...
				case "fixweapons":
					...
					break;
				case "pgb":
					debugOutput = string.Concat("Gem birds: North ", IslandGemBird.GetBirdTypeForLocation("IslandNorth"), " South ", IslandGemBird.GetBirdTypeForLocation("IslandSouth"), " East ", IslandGemBird.GetBirdTypeForLocation("IslandEast"), " West ", IslandGemBird.GetBirdTypeForLocation("IslandWest"));
					break;
				case "ppp":
				case "printplayerpos":
					debugOutput = string.Concat("Player tile position is ", player.getTileLocation(), " (World position: ", player.Position, ")");
					break;
				case "showplurals":
				{
		...
		protected override void Draw(GameTime gameTime)
		{
			...
			if (uiScreen != null)
			{
				base.GraphicsDevice.SetRenderTarget(uiScreen);
		...
		protected virtual void _draw(GameTime gameTime, RenderTarget2D target_screen)
		{
			...
			else
			{
				...
				if (!eventUp && farmEvent == null && currentBillboard == 0 && gameMode == 3 && !takingMapScreenshot && isOutdoorMapSmallerThanViewport())
				{
					...
					spriteBatch.Draw(fadeToBlackRect, new Microsoft.Xna.Framework.Rectangle(0, 0, graphics.GraphicsDevice.Viewport.Width, -Math.Min(viewport.Y, 4096)), Microsoft.Xna.Framework.Color.Black);
					spriteBatch.Draw(fadeToBlackRect, new Microsoft.Xna.Framework.Rectangle(0, -viewport.Y + currentLocation.map.Layers[0].LayerHeight * 64, graphics.GraphicsDevice.Viewport.Width, Math.Min(4096, graphics.GraphicsDevice.Viewport.Height - (-viewport.Y + currentLocation.map.Layers[0].LayerHeight * 64))), Microsoft.Xna.Framework.Color.Black);
				}
				spriteBatch.End();
				PushUIMode();
				spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, null);
		...
		public static void drawDialogueBox(int x, int y, int width, int height, bool speaker, bool drawOnlyBox, string message = null, bool objectDialogueWithPortrait = false, bool ignoreTitleSafe = true, int r = -1, int g = -1, int b = -1)
		{
			...
			if ((objectDialogueWithPortrait && objectDialoguePortraitPerson != null) || (speaker && currentSpeaker != null && currentSpeaker.CurrentDialogue.Count > 0 && currentSpeaker.CurrentDialogue.Peek().showPortrait))
			{
				...
				if (theSpeaker.Portrait != null)
				{
					...
					spriteBatch.Draw(theSpeaker.Portrait, new Vector2(dialogueX + x + 768 + 32, screenHeight - 320 - 64 * addedTileHeightForQuestions - 256 + dialogueY + 16 - 60 + everythingYOffset), portraitRect, Microsoft.Xna.Framework.Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.99f);
				}
				spriteBatch.End();
				spriteBatch.Begin();
				if (isQuestion)
		...
		public static Microsoft.Xna.Framework.Color getColorForTreasureType(string type)
		{
			return type switch
			{
				"Copper" => Microsoft.Xna.Framework.Color.Sienna, 
				"Iron" => Microsoft.Xna.Framework.Color.LightSlateGray, 
				"Coal" => Microsoft.Xna.Framework.Color.Black, 
				"Gold" => Microsoft.Xna.Framework.Color.Gold, 
				"Iridium" => Microsoft.Xna.Framework.Color.Purple, 
				"Coins" => Microsoft.Xna.Framework.Color.Yellow, 
				"Arch" => Microsoft.Xna.Framework.Color.White, 
				_ => Microsoft.Xna.Framework.Color.SaddleBrown, 
			};
		}

		public void _PerformRemoveNormalItemFromWorldOvernight(int parent_sheet_index)
		{
			foreach (GameLocation k in locations)
			{
				_RecursiveRemoveThisNormalItemLocation(k, parent_sheet_index);
			}
			...
			for (int j = 0; j < player.team.returnedDonations.Count; j++)
			{
				if (_RecursiveRemoveThisNormalItemItem(player.team.returnedDonations[j], parent_sheet_index))
				{
					player.team.returnedDonations.RemoveAt(j);
					j--;
				}
			}
