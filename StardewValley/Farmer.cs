	...
	public class Farmer : Character, IComparable
	{
		...
		private void playHarpEmoteSound()
		{
			...
			if (Game1.IsMultiplayer && (long)uniqueMultiplayerID % 111 == 0L)
			{
				...
				{
					DelayedAction.playSoundAfterDelay("miniharp_note", Game1.random.Next(60, 150) * j, base.currentLocation, notes[j]);
			...
			else
			{
				...
				{
					DelayedAction.playSoundAfterDelay("miniharp_note", (i > 0) ? (150 + Game1.random.Next(35, 51) * i) : 0, base.currentLocation, notes[i]);
				}
			}
		}
		...
		public bool hasCurrentOrPendingRoommate()
		{
			...
			if (friendshipData.TryGetValue(spouse, out var friendship))
			{
		...
		public static void showEatingItem(Farmer who)
		{
			...
			{
			case 1:
				tempSprite = ((!who.IsLocalPlayer || who.itemToEat == null || !(who.itemToEat is Object) || !Utility.IsNormalObjectAtParentSheetIndex(who.itemToEat, 434)) ? new TemporaryAnimatedSprite("Maps\\springobjects", Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, (who.itemToEat as Object).parentSheetIndex, 16, 16), 254f, 1, 0, who.Position + new Vector2(-21f, -112f), flicker: false, flipped: false, (float)who.getStandingY() / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f) : new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(368, 16, 16, 16), 62.75f, 8, 2, who.Position + new Vector2(-21f, -112f), flicker: false, flipped: false, (float)who.getStandingY() / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f));
				break;
			case 2:
				...
				{
					tempSprite = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(368, 16, 16, 16), 81.25f, 8, 0, who.Position + new Vector2(-21f, -108f), flicker: false, flipped: false, (float)who.getStandingY() / 10000f + 0.01f, 0f, Color.White, 4f, -0.01f, 0f, 0f)
					{
				...
				tempSprite = new TemporaryAnimatedSprite("Maps\\springobjects", Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, (who.itemToEat as Object).parentSheetIndex, 16, 16), 650f, 1, 0, who.Position + new Vector2(-21f, -108f), flicker: false, flipped: false, (float)who.getStandingY() / 10000f + 0.01f, 0f, Color.White, 4f, -0.01f, 0f, 0f)
				{
					motion = new Vector2(0.8f, -11f),
					acceleration = new Vector2(0f, 0.5f)
				};
			...
			case 4:
			{
				...
				for (int i = 0; i < 8; i++)
				{
					...
					r.Width = 4;
					r.Height = 4;
					tempSprite = new TemporaryAnimatedSprite("Maps\\springobjects", r, 400f, 1, 0, who.Position + new Vector2(24f, -48f), flicker: false, flipped: false, (float)who.getStandingY() / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f)
					{
		...
		public void updateMovementAnimation(GameTime time)
		{
			...
			{
				bool moving = false;
				moving = ((!IsLocalPlayer) ? IsRemoteMoving() : (movementDirections.Count > 0));
				if ((moving && _emoteGracePeriod <= 0) || !FarmerSprite.PauseForSingleAnimation)
				{
					EndEmoteAnimation();
				}
			}
