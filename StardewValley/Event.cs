...
namespace StardewValley
{
	public class Event
	{
		...
		public virtual void tryEventCommand(GameLocation location, GameTime time, string[] split)
		{
			...
			if (_commandLookup.TryGetValue(split[0], out var method_info))
			{
				try
				{
		...
		public virtual void command_warp(GameLocation location, GameTime time, string[] split)
		{
			...
			else if (split[1].Contains("spouse"))
			{
				if (Game1.player.spouse != null && Game1.player.spouse.Length > 0 && getActorByName(Game1.player.spouse) != null && !Game1.player.isRoommate(Game1.player.spouse))
				{
					if (npcControllers != null)
					{
						...
						{
							if (npcControllers[i].puppet.Name.Equals(Game1.player.spouse))
							{
			...
			{
				NPC j = getActorByName(split[1]);
				if (j != null)
				{
					j.position.X = OffsetPositionX(Convert.ToInt32(split[2]) * 64 + 4);
					j.position.Y = OffsetPositionY(Convert.ToInt32(split[3]) * 64);
				}
			}
			CurrentCommand++;
			if (split.Length > 4)
		...
		public virtual void command_animate(GameLocation location, GameTime time, string[] split)
		{
			...
			{
				animationFrames.Add(new FarmerSprite.AnimationFrame(Convert.ToInt32(split[i]), interval, secondaryArm: false, flip));
			}
			if (split[1].Contains("farmer"))
			{
				Farmer f = getFarmerFromFarmerNumberString(split[1], farmer);
			...
			{
				NPC j = getActorByName(split[1].Replace('_', ' '));
				if (j != null)
				{
		...
		public virtual void command_makeInvisible(GameLocation location, GameTime time, string[] split)
		{
			if (split.Count() == 3)
			{
				...
				Object o2 = null;
				o2 = location.getObjectAtTile(x, y2);
				if (o2 != null)
				{
			...
			else
			{
				...
				for (int i = x2; i < x2 + width; i++)
				{
					...
					{
						o = location.getObjectAtTile(i, j);
						if (o != null)
						{
		...
		public virtual void command_fork(GameLocation location, GameTime time, string[] split)
		{
			if (split.Length > 2)
			{
				if (Game1.player.mailReceived.Contains(split[1]) || (int.TryParse(split[1], out var i) && Game1.player.dialogueQuestionsAnswered.Contains(i)))
				{
		...
		public virtual void command_elliottbooktalk(GameLocation location, GameTime time, string[] split)
		{
			...
			{
				string speech = "";
				speech = (Game1.player.dialogueQuestionsAnswered.Contains(958699) ? Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1257") : (Game1.player.dialogueQuestionsAnswered.Contains(958700) ? Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1258") : ((!Game1.player.dialogueQuestionsAnswered.Contains(9586701)) ? Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1260") : Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1259"))));
				NPC i = getActorByName("Elliott");
				if (i == null)
				{
					i = Game1.getCharacterFromName("Elliott");
				}
				i.CurrentDialogue.Push(new Dialogue(speech, i));
				Game1.drawDialogue(i);
			}
		}
		...
		public virtual void command_drawOffset(GameLocation location, GameTime time, string[] split)
		{
			...
			character = (Character)((!split[1].Equals("farmer")) ? ((object)getActorByName(split[1])) : ((object)farmer));
			character.drawOffset.Value = new Vector2(x_offset, y_offset) * 4f;
			CurrentCommand++;
		}
		...
		public virtual void command_animateHeight(GameLocation location, GameTime time, string[] split)
		{
			...
			{
				character.yJumpGravity = jump_gravity.Value;
			}
			if (jump_velocity.HasValue)
			{
				character.yJumpVelocity = jump_velocity.Value;
			}
			CurrentCommand++;
		}
			...
			if (temporaryLocation != null && temporaryLocation.map.GetLayer(split[1]) != null)
			{
				...
				if (festivalData != null)
				{
					...
					if (festivalData.ContainsKey(key_name))
					{
						...
						foreach (string additional_character_data in array)
						{
							...
							if (!fail)
							{
								string direction_string = additional_character_split[3];
								direction_string = direction_string.ToLowerInvariant();
								...
								case "right":
									...
									break;
								default:
									if (!int.TryParse(direction_string, out direction))
									{
										fail = true;
									}
									break;
				...
				if (split[1] == "Set-Up")
				{
					foreach (string npc_name in npc_names)
					{
						...
						npc = getCharacterByName(npc_name) as NPC;
						for (int child_index = 0; child_index < children.Count; child_index++)
						{
							...
							child_actor.Halt();
							new Point((int)npc.Position.X / 64, (int)npc.Position.Y / 64);
							Point[] direction_offsets = null;
							direction_offsets = npc.FacingDirection switch
							{
								0 => new Point[4]
								{
									new Point(0, 1),
							...
							Point spawn_point = new Point(npc.getTileX(), npc.getTileY());
							List<Point> points_to_check = new List<Point>();
							new List<Point>();
							Point[] array2 = direction_offsets;
							...
							if (found_spawn)
							{
								...
								child_actor.DefaultPosition = npc.DefaultPosition;
								child_actor.faceDirection(npc.FacingDirection);
		...
		public void endBehaviors(string[] split, GameLocation location)
		{
			...
			if (split != null && split.Length > 1)
			{
				...
				case "warpOut":
				{
					...
					setExitLocation(location.warps[whichWarp].TargetName, location.warps[whichWarp].TargetX, location.warps[whichWarp].TargetY);
					Game1.eventOver = true;
					CurrentCommand += 2;
					Game1.screenGlowHold = false;
					break;
		...
		protected void _checkForNextCommand(GameLocation location, GameTime time)
		{
			...
			if (npcControllers != null)
			{
				...
				{
					npcControllers[j].puppet.isCharging = !isFestival;
					if (npcControllers[j].update(time, location, npcControllers))
					{
			...
			if (!Game1.fadeToBlack || actorPositionsAfterMove.Count > 0 || CurrentCommand > 3 || forked)
			{
				...
				if (actorPositionsAfterMove.Count > 0)
				{
					...
					foreach (string s in array)
					{
						...
						foreach (NPC k in actors)
						{
							Microsoft.Xna.Framework.Rectangle r = k.GetBoundingBox();
							if (k.Name.Equals(s) && targetTile.Contains(r) && k.GetBoundingBox().Y - targetTile.Top <= 16)
							{
								k.Halt();
		...
		private void populateWalkLocationsList()
		{
			...
			for (int j = 2; j < eventCommands.Length; j++)
			{
				string[] split = eventCommands[j].Split(' ');
				string a = split[0];
		...
		private void trySpecialSetUp(GameLocation location)
		{
			...
			case 4324303:
			{
				...
				if (!room.Contains(Game1.player.getTileLocationPoint()))
				{
					...
					commands.Insert(command_index, "beginSimultaneousCommand");
					command_index++;
					commands.Insert(command_index, "viewport " + Game1.player.getTileX() + " " + Game1.player.getTileY());
					command_index++;
					commands.Insert(command_index, "globalFadeToClear 0.03");
					command_index++;
					commands.Insert(command_index, "endSimultaneousCommand");
		...
		private void setUpCharacters(string description, GameLocation location)
		{
			...
			for (int i = 0; i < split.Length; i += 4)
			{
				...
				{
					foreach (NPC j in location.getCharacters())
					{
				...
				else if (!split[i].Equals("farmer"))
				{
					if (split[i].Equals("otherFarmers"))
					{
						int x2 = OffsetTileX(Convert.ToInt32(split[i + 1]));
						int y2 = OffsetTileY(Convert.ToInt32(split[i + 2]));
						int direction2 = Convert.ToInt32(split[i + 3]);
					...
					if (split[i].Contains("farmer"))
					{
						int x = OffsetTileX(Convert.ToInt32(split[i + 1]));
						int y = OffsetTileY(Convert.ToInt32(split[i + 2]));
						int direction = Convert.ToInt32(split[i + 3]);
						Farmer f = Utility.getFarmerFromFarmerNumber(Convert.ToInt32(split[i].Last().ToString() ?? ""));
						if (f != null)
						{
					...
					{
						actors.Add(new Cat(OffsetTileX(Convert.ToInt32(split[i + 1])), OffsetTileY(Convert.ToInt32(split[i + 2])), Game1.player.whichPetBreed));
					...
					{
						actors.Add(new Dog(OffsetTileX(Convert.ToInt32(split[i + 1])), OffsetTileY(Convert.ToInt32(split[i + 2])), Game1.player.whichPetBreed));
						actors.Last().Name = "Dog";
						actors.Last().position.X -= 42f;
						continue;
					}
					if (split[i].Equals("golem"))
					{
						actors.Add(new NPC(new AnimatedSprite("Characters\\Monsters\\Wilderness Golem", 0, 16, 24), OffsetPosition(new Vector2(Convert.ToInt32(split[i + 1]), Convert.ToInt32(split[i + 2])) * 64f), 0, "Golem"));
						continue;
					}
					if (split[i].Equals("Junimo"))
					{
						actors.Add(new Junimo(OffsetPosition(new Vector2(Convert.ToInt32(split[i + 1]) * 64, Convert.ToInt32(split[i + 2]) * 64 - 32)), Game1.currentLocation.Name.Equals("AbandonedJojaMart") ? 6 : (-1))
						{
							Name = "Junimo",
							EventActor = true
						});
					...
					int xPos = OffsetTileX(Convert.ToInt32(split[i + 1]));
					int yPos = OffsetTileY(Convert.ToInt32(split[i + 2]));
				...
				{
					farmer.position.X = OffsetPositionX(Convert.ToInt32(split[i + 1]) * 64);
					farmer.position.Y = OffsetPositionY(Convert.ToInt32(split[i + 2]) * 64 + 16);
					farmer.faceDirection(Convert.ToInt32(split[i + 3]));
					if (location is Farm && id != -2 && !ignoreTileOffsets)
					{
		...
		private void addSpecificTemporarySprite(string key, GameLocation location, string[] split)
		{
			...
			case "harveyDinnerSet":
			{
				...
				Object o = null;
				o = location.getObjectAtTile((int)centerPoint.X, (int)centerPoint.Y);
				if (o != null)
				{
					o.isTemporarilyInvisible = true;
				}
		...
		private void setUpSecretSantaCommands()
		{
			...
			try
			{
				...
				secretSantaY = getActorByName(mySecretSanta.Name).getTileY();
			}
			catch (Exception)
			{
			...
			{
			case 2:
				dialogue = Game1.LoadStringByGender(mySecretSanta.gender, "Strings\\StringsFromCSFiles:Event.cs.1497");
			...
			case 1:
				...
				case 0:
				case 1:
					dialogue = Game1.LoadStringByGender(mySecretSanta.gender, "Strings\\StringsFromCSFiles:Event.cs.1499");
					dialogue2 = Game1.LoadStringByGender(mySecretSanta.gender, "Strings\\StringsFromCSFiles:Event.cs.1500");
					break;
				case 2:
					dialogue = Game1.LoadStringByGender(mySecretSanta.gender, "Strings\\StringsFromCSFiles:Event.cs.1501");
			...
			for (int i = 0; i < eventCommands.Length; i++)
			{
				...
				eventCommands[i] = eventCommands[i].Replace("warpY", string.Concat(secretSantaY));
				eventCommands[i] = eventCommands[i].Replace("dialogue1", dialogue);
		...
		public void setUpFestivalMainEvent()
		{
			...
			string[] newCommands = (eventCommands = rawFestivalData.Split('/'));
		}
		...
		public bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
		{
			if (isFestival)
			{
				...
				string tileSheetID = "";
				int tileIndex = -1;
				tileIndex = Game1.currentLocation.getTileIndexAt(tileLocation.X, tileLocation.Y, "Buildings");
				tileAction = Game1.currentLocation.doesTileHaveProperty(tileLocation.X, tileLocation.Y, "Action", "Buildings");
				tileSheetID = Game1.currentLocation.getTileSheetIDAt(tileLocation.X, tileLocation.Y, "Buildings");
				if (Game1.currentSeason == "winter" && Game1.dayOfMonth == 8 && tileSheetID == "fest" && (tileIndex == 1009 || tileIndex == 1010 || tileIndex == 1012 || tileIndex == 1013))
				{
					Game1.playSound("pig");
					return true;
				}
				...
				case 175:
				case 176:
					if (tileSheetID == "untitled tile sheet" && who.IsLocalPlayer && festivalData["file"].Equals("fall16"))
					{
						...
						case "Shop":
						{
							...
							if (!festivalShops.ContainsKey(split[1]))
							{
								...
								for (int j = 0; j < inventoryList.Length; j += 4)
								{
									...
									int stock = Convert.ToInt32(inventoryList[j + 3]);
									Item item = null;
									switch (type)
									{
									case "Object":
								...
								festivalShops.Add(split[1], stockList);
							}
							else
							{
								stockList = festivalShops[split[1]];
							}
							if (stockList != null && stockList.Count > 0)
							{
								Game1.activeClickableMenu = new ShopMenu(stockList);
							}
							else
							{
				...
				else if (isFestival)
				{
					if (who.IsLocalPlayer)
					{
						foreach (NPC k in actors)
						{
							if (k.getTileX() == tileLocation.X && k.getTileY() == tileLocation.Y && k is Child)
							{
								(k as Child).checkAction(who, temporaryLocation);
								return true;
							}
							if (k.getTileX() != tileLocation.X || k.getTileY() != tileLocation.Y || (k.CurrentDialogue.Count() < 1 && (k.CurrentDialogue.Count() <= 0 || k.CurrentDialogue.Peek().isOnFinalDialogue()) && !k.Equals(festivalHost) && (!k.datable || !festivalData["file"].Equals("spring24")) && (secretSantaRecipient == null || !k.Name.Equals(secretSantaRecipient.Name))))
							{
								continue;
							}
							bool divorced = who.friendshipData.ContainsKey(k.Name) && who.friendshipData[k.Name].IsDivorced();
							if ((grangeScore > -100 || grangeScore == -666) && k.Equals(festivalHost) && grangeJudged)
							{
								...
								if (grangeScore >= 90)
								{
									...
									message = Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1723", grangeScore);
									Game1.player.festivalScore += 1000;
								}
								else if (grangeScore >= 75)
								{
									Game1.playSound("reward");
									message = Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1726", grangeScore);
									Game1.player.festivalScore += 500;
								}
								else if (grangeScore >= 60)
								{
									Game1.playSound("newArtifact");
									message = Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1729", grangeScore);
									Game1.player.festivalScore += 250;
								}
								else if (grangeScore == -666)
								{
									Game1.playSound("secret1");
									message = Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1730");
									Game1.player.festivalScore += 750;
								}
								else
								{
									Game1.playSound("newArtifact");
									message = Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1732", grangeScore);
									Game1.player.festivalScore += 50;
								}
								grangeScore = -100;
								k.setNewDialogue(message);
							}
							else if ((Game1.serverHost == null || Game1.player.Equals(Game1.serverHost.Value)) && k.Equals(festivalHost) && (k.CurrentDialogue.Count() == 0 || k.CurrentDialogue.Peek().isOnFinalDialogue()) && hostMessage != null)
							{
								k.setNewDialogue(hostMessage);
							}
							else if ((Game1.serverHost == null || Game1.player.Equals(Game1.serverHost.Value)) && k.Equals(festivalHost) && (k.CurrentDialogue.Count == 0 || k.CurrentDialogue.Peek().isOnFinalDialogue()) && hostMessage != null)
							{
								k.setNewDialogue(hostMessage);
							}
							if (isSpecificFestival("spring24") && !divorced && ((bool)k.datable || (who.spouse != null && k.Name.Equals(who.spouse))))
							{
								...
								if (who.dancePartner.Value == null)
								{
									...
									if (k.CurrentDialogue.Count == 0)
									{
										...
										{
											k.setNewDialogue(Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1736", k.displayName), add: true);
										}
										else
										{
											k.setNewDialogue(Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1738", k.displayName), add: true);
										}
									}
							...
							if (who.spouse != null && k.Name.Equals(who.spouse) && !festivalData["file"].Equals("spring24") && festivalData.ContainsKey(k.Name + "_spouse"))
							{
								k.CurrentDialogue.Clear();
								k.CurrentDialogue.Push(new Dialogue(GetFestivalDataForYear(k.Name + "_spouse"), k));
							}
							if (divorced)
							{
								k.CurrentDialogue.Clear();
								k.CurrentDialogue.Push(new Dialogue(Game1.content.Load<Dictionary<string, string>>("Characters\\Dialogue\\" + k.Name)["divorced"], k));
							}
							k.grantConversationFriendship(who);
							Game1.drawDialogue(k);
							k.faceTowardFarmerForPeriod(3000, 2, faceAway: false, who);
		...
		public void setUpAdvancedMove(string[] split, NPCController.endBehavior endBehavior = null)
		{
			...
			{
				path.Add(new Vector2(Convert.ToInt32(split[i]), Convert.ToInt32(split[i + 1])));
			}
			if (split[1].Contains("farmer"))
			{
				Farmer f = getFarmerFromFarmerNumberString(split[1], farmer);
			...
			if (j != null)
			{
				npcControllers.Add(new NPCController(j, path, Convert.ToBoolean(split[2]), endBehavior));
			}
		}
		...
		private void eggHuntWinner()
		{
			...
			getActorByName("Lewis").CurrentDialogue.Push(new Dialogue(winnerDialogue, getActorByName("Lewis")));
		...
		private void iceFishingWinner()
		{
			...
			if (mostFishScore >= numberOfFishToWin)
			{
				if (winners.Count == 1)
				{
					winnerDialogue = Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1872", winners[0].displayName, winners[0].festivalScore);
				}
				else
				{
					winnerDialogue = Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1864");
