...
namespace StardewValley
{
	public class Buff
	{
		...
		public void addBuff()
		{
			...
			{
				int buff_count = 0;
				buff_count = ((!Game1.player.appliedSpecialBuffs.TryGetValue(which, out buff_count)) ? 1 : (buff_count + 1));
				Game1.player.appliedSpecialBuffs[which] = buff_count;
			}
			_ = glow;
		...
		public void removeBuff()
		{
			...
			{
				int buff_stacks = 0;
				if (Game1.player.appliedSpecialBuffs.TryGetValue(which, out buff_stacks))
				{
					buff_stacks--;
					if (buff_stacks > 1)
					{
						buff_stacks--;
						Game1.player.appliedSpecialBuffs[which] = buff_stacks;
					}
					else
					{
