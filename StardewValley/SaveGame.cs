...
namespace StardewValley
{
	public class SaveGame
	{
		...
		public static bool IsNewGameSaveNameCollision(string save_name)
		{
			...
			{
				friendlyName += Path.DirectorySeparatorChar;
			}
		...
		public static void loadDataToLocations(List<GameLocation> gamelocations)
		{
			...
			{
				if (l3 is Cellar && Game1.getLocationFromName(l3.name) == null)
				{
					Game1.locations.Add(new Cellar("Maps\\Cellar", l3.name));
				}
				if (l3 is FarmHouse)
				{
					GameLocation realLocation2 = Game1.getLocationFromName(l3.name);
					(realLocation2 as FarmHouse).setMapForUpgradeLevel((realLocation2 as FarmHouse).upgradeLevel);
					(realLocation2 as FarmHouse).fireplaceOn.Value = (l3 as FarmHouse).fireplaceOn;
					(realLocation2 as FarmHouse).fridge.Value = (l3 as FarmHouse).fridge;
					(realLocation2 as FarmHouse).farmerNumberOfOwner = (l3 as FarmHouse).farmerNumberOfOwner;
				}
				if (l3.name.Equals("Farm"))
				{
					GameLocation realLocation3 = Game1.getLocationFromName(l3.name);
					foreach (Building building in ((Farm)l3).buildings)
					{
						building.load();
					}
					((Farm)realLocation3).buildings.Set(((Farm)l3).buildings);
					foreach (FarmAnimal value in ((Farm)l3).animals.Values)
					{
						value.reload(null);
					}
					((Farm)realLocation3).greenhouseUnlocked.Value = ((Farm)l3).greenhouseUnlocked.Value;
					((Farm)realLocation3).greenhouseMoved.Value = ((Farm)l3).greenhouseMoved.Value;
				}
				if (l3.name.Equals("MovieTheater"))
				{
					(Game1.getLocationFromName(l3.name) as MovieTheater).dayFirstEntered.Set((l3 as MovieTheater).dayFirstEntered);
				}
			}
			...
			foreach (GameLocation l4 in gamelocations)
			{
				GameLocation realLocation = Game1.getLocationFromName(l4.name);
				if (realLocation == null)
				{
					continue;
				}
				realLocation.miniJukeboxCount.Value = l4.miniJukeboxCount.Value;
				realLocation.miniJukeboxTrack.Value = l4.miniJukeboxTrack.Value;
				realLocation.furniture.Set(l4.furniture);
				foreach (Furniture item in realLocation.furniture)
				{
					item.updateDrawPosition();
				}
				for (int k = l4.characters.Count - 1; k >= 0; k--)
				{
					initializeCharacter(l4.characters[k], realLocation);
				}
				foreach (LargeTerrainFeature largeTerrainFeature in l4.largeTerrainFeatures)
				{
					largeTerrainFeature.currentLocation = realLocation;
					largeTerrainFeature.loadSprite();
				}
				foreach (TerrainFeature value2 in l4.terrainFeatures.Values)
				{
					value2.currentLocation = realLocation;
					value2.loadSprite();
				}
				foreach (KeyValuePair<Vector2, Object> v in l4.objects.Pairs)
				{
					v.Value.initializeLightSource(v.Key);
					v.Value.reloadSprite();
				}
				if (l4.name.Equals("Farm"))
				{
					((Farm)realLocation).buildings.Set(((Farm)l4).buildings);
					foreach (FarmAnimal value3 in ((Farm)l4).animals.Values)
					{
						value3.reload(null);
					}
				...
				realLocation.characters.Set(l4.characters);
				realLocation.netObjects.Set(l4.netObjects.Pairs);
				realLocation.numberOfSpawnedObjectsOnMap = l4.numberOfSpawnedObjectsOnMap;
				realLocation.terrainFeatures.Set(l4.terrainFeatures.Pairs);
				realLocation.largeTerrainFeatures.Set(l4.largeTerrainFeatures);
				if (realLocation.name.Equals("Farm"))
				{
					((Farm)realLocation).animals.MoveFrom(((Farm)l4).animals);
					(realLocation as Farm).piecesOfHay.Value = (l4 as Farm).piecesOfHay;
					List<ResourceClump> clumps = new List<ResourceClump>((l4 as Farm).resourceClumps);
					(l4 as Farm).resourceClumps.Clear();
					(realLocation as Farm).resourceClumps.Set(clumps);
					(realLocation as Farm).hasSeenGrandpaNote = (l4 as Farm).hasSeenGrandpaNote;
					(realLocation as Farm).grandpaScore = (l4 as Farm).grandpaScore;
					(realLocation as Farm).petBowlWatered.Set((l4 as Farm).petBowlWatered.Value);
				}
				if (realLocation.name.Equals("Town"))
				{
					(realLocation as Town).daysUntilCommunityUpgrade.Value = (l4 as Town).daysUntilCommunityUpgrade;
				}
				if (realLocation is Beach)
				{
					(realLocation as Beach).bridgeFixed.Value = (l4 as Beach).bridgeFixed;
				}
				if (realLocation is Woods)
				{
					(realLocation as Woods).stumps.MoveFrom((l4 as Woods).stumps);
					(realLocation as Woods).hasUnlockedStatue.Value = (l4 as Woods).hasUnlockedStatue.Value;
				}
				if (realLocation is CommunityCenter)
				{
					(realLocation as CommunityCenter).areasComplete.Set((l4 as CommunityCenter).areasComplete);
				}
				if (realLocation is ShopLocation && l4 is ShopLocation)
				{
					(realLocation as ShopLocation).itemsFromPlayerToSell.MoveFrom((l4 as ShopLocation).itemsFromPlayerToSell);
					(realLocation as ShopLocation).itemsToStartSellingTomorrow.MoveFrom((l4 as ShopLocation).itemsToStartSellingTomorrow);
				}
				if (realLocation is Forest)
				{
					...
					(realLocation as Forest).log = (l4 as Forest).log;
				}
				realLocation.TransferDataFromSavedLocation(l4);
			...
			{
				for (int j = 0; j < l2.characters.Count; j++)
				{
					characters.Add(l2.characters[j]);
				}
			}
			for (int i = 0; i < characters.Count; i++)
			{
				characters[i].reloadSprite();
			}
			foreach (GameLocation n in Game1.locations)
			{
				if (!(n is BuildableGameLocation))
				{
					continue;
				}
				foreach (Building building3 in (n as BuildableGameLocation).buildings)
				{
					...
					for (int l = interior.characters.Count - 1; l >= 0; l--)
					{
						if (l < interior.characters.Count)
						{
							interior.characters[l].reloadSprite();
						}
					}
				}
			}
			foreach (GameLocation m in Game1.locations)
			{
				...
				Game1.getLocationFromName(m.name);
				foreach (Building b in ((Farm)m).buildings)
				{
