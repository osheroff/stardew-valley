...
namespace StardewValley
{
	...
	public abstract class Item : IComparable, INetObject<NetFields>, ISalable
	{
		...
		protected void _GenerateContextTags()
		{
			...
			string alternate_id = "id_" + Utility.getStandardDescriptionFromItem(this, 0, '_');
			alternate_id = alternate_id.Substring(0, alternate_id.Length - 2).ToLower();
			_contextTags.Add(alternate_id);
			...
			else if (Name != null && Game1.objectContextTags.ContainsKey(alternate_id))
			{
				string[] array = Game1.objectContextTags[alternate_id].Split(',');
				for (int i = 0; i < array.Length; i++)
				{
