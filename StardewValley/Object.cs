...
namespace StardewValley
{
	...
	public class Object : Item
	{
			...
			: this()
		{
			...
			Game1.bigCraftablesInformation.TryGetValue(parentSheetIndex, out var objectInformation);
			...
			: this()
		{
			this.tileLocation.Value = tileLocation;
			base.ParentSheetIndex = parentSheetIndex;
			Game1.objectInformation.TryGetValue(parentSheetIndex, out var objectInformation);
			try
			{
				if (objectInformation != null)
				{
		...
		protected virtual string loadDisplayName()
		{
			...
			{
				Game1.objectInformation.TryGetValue(preservedParentSheetIndex, out var objectInformation4);
			...
			else
			{
				if (name != null && name.Contains("Honey"))
				{
					...
					if (preservedParentSheetIndex.Value == 0)
					{
						Game1.objectInformation.TryGetValue(parentSheetIndex, out var objectInformation3);
				...
				{
					Game1.bigCraftablesInformation.TryGetValue(parentSheetIndex, out var objectInformation2);
		...
		public virtual bool performToolAction(Tool t, GameLocation location)
		{
			...
			if (name.Contains("SupplyCrate") && t.isHeavyHitter())
			{
				...
				if (MinutesUntilReady <= 0)
				{
					...
					Random r = new Random((int)((float)Game1.uniqueIDForThisGame + tileLocation.X * 777f + tileLocation.Y * 7f));
			...
			if ((bool)bigCraftable)
			{
				...
				case 128:
					if (heldObject.Value == null)
					{
						...
						whichMushroom = ((Game1.random.NextDouble() < 0.025) ? 422 : ((Game1.random.NextDouble() < 0.075) ? 281 : ((Game1.random.NextDouble() < 0.09) ? 257 : ((!(Game1.random.NextDouble() < 0.15)) ? 404 : 420))));
						heldObject.Value = new Object(whichMushroom, 1);
			...
			if (dropInItem is Object)
			{
				...
				else if (name.Equals("Preserves Jar"))
				{
					...
					case 812:
					{
						...
						Object aged_roe = null;
						ColoredObject colored_object;
						aged_roe = (((colored_object = dropIn as ColoredObject) == null) ? new Object(447, 1) : new ColoredObject(447, 1, colored_object.color));
						heldObject.Value = aged_roe;
			...
			else
			{
				if (name != null && name.Contains("Secret Note"))
				{
					...
					if (!Game1.player.secretNotesSeen.Contains(which2))
					{
						...
						if (which2 == 23 && !Game1.player.eventsSeen.Contains(2120303))
						{
							Game1.player.addQuest(29);
						}
						else if (which2 == 10 && !Game1.player.mailReceived.Contains("qiCave"))
						{
				...
				{
					int which = ((!name.Contains('#')) ? 1 : Convert.ToInt32(name.Split('#')[1]));
					which += GameLocation.JOURNAL_INDEX;
					if (!Game1.player.secretNotesSeen.Contains(which))
					{
						Game1.player.secretNotesSeen.Add(which);
					}
		...
		public virtual bool checkForAction(Farmer who, bool justCheckingForActivity = false)
		{
			...
			if ((bool)bigCraftable)
			{
				...
				case 238:
				{
					...
					Vector2 target = ((!(Vector2.Distance(who.getTileLocation(), obelisk1) > Vector2.Distance(who.getTileLocation(), obelisk2))) ? obelisk2 : obelisk1);
						...
						else if (name.Contains("Feed Hopper") && who.ActiveObject == null)
						{
							...
							if (who.freeSpotsInInventory() > 0)
							{
								...
								if (piecesHay > 0)
								{
									...
									{
										int piecesOfHayToRemove = Math.Min((who.currentLocation as AnimalHouse).animalsThatLiveHere.Count, piecesHay);
										piecesOfHayToRemove = Math.Max(1, piecesOfHayToRemove);
										AnimalHouse k = who.currentLocation as AnimalHouse;
										int alreadyHay = k.numberOfObjectsWithName("Hay");
										piecesOfHayToRemove = Math.Min(piecesOfHayToRemove, (int)k.animalLimit - alreadyHay);
										if (piecesOfHayToRemove != 0 && Game1.player.couldInventoryAcceptThisObject(178, piecesOfHayToRemove))
										{
											(Game1.getLocationFromName("Farm") as Farm).piecesOfHay.Value -= Math.Max(1, piecesOfHayToRemove);
											who.addItemToInventoryBool(new Object(178, piecesOfHayToRemove));
		...
		public virtual int sellToStorePrice(long specificPlayerID = -1L)
		{
			...
			float salePrice = (int)((float)(int)price * (1f + (float)Quality * 0.25f));
