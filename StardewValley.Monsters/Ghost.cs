		...
		protected override void updateAnimation(GameTime time)
		{
			...
			if (nextParticle <= 0f)
			{
				...
				if (variant.Value == GhostVariant.Putrid)
				{
					...
					{
						TemporaryAnimatedSprite drip = new TemporaryAnimatedSprite(layerDepth: getStandingPosition().Y / 10000f, textureName: Sprite.textureName, sourceRect: new Microsoft.Xna.Framework.Rectangle(Game1.random.Next(4) * 16, 168, 16, 24), animationInterval: 100f, animationLength: 1, numberOfLoops: 10, position: base.Position + new Vector2(Utility.RandomFloat(-16f, 16f), Utility.RandomFloat(-16f, 0f) - (float)yOffset), flicker: false, flipped: false, alphaFade: 0.01f, color: Color.White, scale: 4f, scaleChange: -0.01f, rotation: 0f, rotationChange: 0f);
