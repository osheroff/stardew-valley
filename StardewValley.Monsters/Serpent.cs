	...
	public class Serpent : Monster
	{
		...
		protected override void localDeathAnimation()
		{
			...
			if (IsRoyalSerpent())
			{
				...
				{
					Vector2 segment_position = Vector2.Zero;
					Rectangle source_rect = new Rectangle(0, 0, 32, 32);
					...
					TemporaryAnimatedSprite current_sprite = null;
					current_sprite = new TemporaryAnimatedSprite(Sprite.textureName, source_rect, 800f, 1, 0, segment_position, flicker: false, flipped: false, 0.9f, 0.001f, segment_color, 4f * (float)scale, 0.01f, current_rotation + (float)Math.PI, (float)((double)Game1.random.Next(3, 5) * Math.PI / 64.0))
					{
		...
		public override void drawAboveAllLayers(SpriteBatch b)
		{
			...
			for (int i = -1; i < segmentCount.Value; i++)
			{
				...
				{
					sort_offset += 0f - ((float)(getStandingY() - 1) / 10000f + max_offset);
				}
				...
				if (Utility.isOnScreen(draw_position, 128))
				{
					...
					b.Draw(Game1.shadowTexture, local_shadow_position + new Vector2(64f, GetBoundingBox().Height), Game1.shadowTexture.Bounds, Color.White, 0f, new Vector2(Game1.shadowTexture.Bounds.Center.X, Game1.shadowTexture.Bounds.Center.Y), 4f, SpriteEffects.None, (float)(getStandingY() - 1) / 10000f + sort_offset);
					b.Draw(Sprite.Texture, local_draw_position + new Vector2(64f, GetBoundingBox().Height / 2), draw_rect, Color.White, current_rotation, new Vector2(16f, 16f), Math.Max(0.2f, scale) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)(getStandingY() + 8) / 10000f + sort_offset)));
					if (isGlowing)
					{
						b.Draw(Sprite.Texture, local_draw_position + new Vector2(64f, GetBoundingBox().Height / 2), draw_rect, glowingColor * glowingTransparency, current_rotation, new Vector2(16f, 16f), Math.Max(0.2f, scale) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)(getStandingY() + 8) / 10000f + 0.0001f + sort_offset)));
					}
					if (is_royal)
					{
						sort_offset += -5E-05f;
						...
						if (i > 0)
						{
							b.Draw(Game1.shadowTexture, local_draw_position + new Vector2(64f, GetBoundingBox().Height), Game1.shadowTexture.Bounds, Color.White, 0f, new Vector2(Game1.shadowTexture.Bounds.Center.X, Game1.shadowTexture.Bounds.Center.Y), 4f, SpriteEffects.None, (float)(getStandingY() - 1) / 10000f + sort_offset);
						}
						b.Draw(Sprite.Texture, local_draw_position + new Vector2(64f, GetBoundingBox().Height / 2), draw_rect, Color.White, current_rotation, new Vector2(16f, 16f), Math.Max(0.2f, scale) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)(getStandingY() + 8) / 10000f + sort_offset)));
						if (isGlowing)
						{
