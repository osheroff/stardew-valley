	...
	public class TailoringMenu : MenuWithInventory
	{
		...
		public Item CraftItem(Item left_item, Item right_item)
		{
			...
			if (recipe != null)
			{
				...
				Item crafted_item = null;
				crafted_item = ((crafted_item_id < 0) ? new Object(-crafted_item_id, 1) : ((crafted_item_id < 2000 || crafted_item_id >= 3000) ? ((Item)new Clothing(crafted_item_id)) : ((Item)new Hat(crafted_item_id - 2000))));
				if (crafted_item != null && crafted_item is Clothing)
				{
					DyeItems(crafted_item as Clothing, right_item, 1f);
				}
				if (crafted_item is Object)
				{
					Object crafted_object = crafted_item as Object;
		...
		{
			Console.WriteLine(string.Concat("meow:", oldBounds, " ", newBounds, " ", width, " ", height, " ", yPositionOnScreen));
