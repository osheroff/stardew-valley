	...
	public class SkillsPage : IClickableMenu
	{
			...
			: base(x, y, width, height)
		{
			...
			for (int k = 4; k < 10; k += 5)
			{
				for (int m = 0; m < 5; m++)
				{
					...
					if (drawRed && (k + 1) % 5 == 0)
					{
						skillBars.Add(new ClickableTextureComponent(string.Concat(professionNumber), new Rectangle(addedX + drawX - 4 + k * 36, drawY + m * 56, 56, 36), null, professionBlurb, Game1.mouseCursors, new Rectangle(159, 338, 14, 9), 4f, drawShadow: true)
						{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				skillAreas.Add(new ClickableTextureComponent(string.Concat(index), new Rectangle(drawX - 128 - 48, drawY + i * 56, 148, 36), string.Concat(index), text, null, Rectangle.Empty, 1f)
				{
					myID = i,
		...
		public override void draw(SpriteBatch b)
		{
			...
			if ((int)Game1.netWorldState.Value.GoldenWalnuts > 0)
			{
				...
				x += 48;
				b.DrawString(Game1.smallFont, string.Concat((object)Game1.netWorldState.Value.GoldenWalnuts), new Vector2(x, y), Game1.textColor);
				x += 64;
			}
			if (Game1.player.QiGems > 0)
			{
				...
				x += 48;
				b.DrawString(Game1.smallFont, string.Concat(Game1.player.QiGems), new Vector2(x, y), Game1.textColor);
				x += 64;
			}
