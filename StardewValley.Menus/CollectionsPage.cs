	...
	public class CollectionsPage : IClickableMenu
	{
			...
			: base(x, y, width, height)
		{
			...
			foreach (KeyValuePair<int, string> kvp2 in list)
			{
				...
				int xPos4 = baseX + widthUsed[whichCollection] % collectionWidth * 68;
				int yPos4 = baseY + widthUsed[whichCollection] / collectionWidth * 68;
				...
				collections[whichCollection].Last().Add(new ClickableTextureComponent(kvp2.Key + " " + farmerHas2.ToString() + " " + farmerHasButNotMade.ToString(), new Rectangle(xPos4, yPos4, 64, 64), null, "", Game1.objectSpriteSheet, Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, kvp2.Key, 16, 16), 4f, farmerHas2)
				{
					myID = collections[whichCollection].Last().Count,
					rightNeighborID = (((collections[whichCollection].Last().Count + 1) % collectionWidth == 0) ? (-1) : (collections[whichCollection].Last().Count + 1)),
					leftNeighborID = ((collections[whichCollection].Last().Count % collectionWidth == 0) ? 7001 : (collections[whichCollection].Last().Count - 1)),
					downNeighborID = ((yPos4 + 68 > yPositionOnScreen + height - 128) ? (-7777) : (collections[whichCollection].Last().Count + collectionWidth)),
					upNeighborID = ((collections[whichCollection].Last().Count < collectionWidth) ? 12345 : (collections[whichCollection].Last().Count - collectionWidth)),
					fullyImmutable = true
				});
		...
		public string createDescription(int index)
		{
			...
			if (currentTab == 5)
			{
				...
				description = description + split3[0] + Environment.NewLine + Environment.NewLine;
				description += split3[1];
			}
			...
			else
			{
				...
				description = description + displayName + Environment.NewLine + Environment.NewLine + Game1.parseText(split[5], Game1.smallFont, 256) + Environment.NewLine + Environment.NewLine;
				if (split[3].Contains("Arch"))
				{
					description += (Game1.player.archaeologyFound.ContainsKey(index) ? Game1.content.LoadString("Strings\\UI:Collections_Description_ArtifactsFound", Game1.player.archaeologyFound[index][0]) : "");
				}
				else if (split[3].Contains("Cooking"))
				{
					description += (Game1.player.recipesCooked.ContainsKey(index) ? Game1.content.LoadString("Strings\\UI:Collections_Description_RecipesCooked", Game1.player.recipesCooked[index]) : "");
				...
				{
					description = ((!split[3].Contains("Minerals") && !split[3].Substring(split[3].Length - 3).Equals("-2")) ? (description + Game1.content.LoadString("Strings\\UI:Collections_Description_NumberShipped", Game1.player.basicShipped.ContainsKey(index) ? Game1.player.basicShipped[index] : 0)) : (description + Game1.content.LoadString("Strings\\UI:Collections_Description_MineralsFound", Game1.player.mineralsFound.ContainsKey(index) ? Game1.player.mineralsFound[index] : 0)));
				}
				else
				{
					description += Game1.content.LoadString("Strings\\UI:Collections_Description_FishCaught", Game1.player.fishCaught.ContainsKey(index) ? Game1.player.fishCaught[index][0] : 0);
