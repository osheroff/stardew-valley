...
namespace StardewValley.Menus
{
	public class QuestLog : IClickableMenu
	{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (questPage == -1)
			{
				for (int j = 0; j < questLogButtons.Count; j++)
				{
					if (pages.Count() > 0 && pages[currentPage].Count() > j)
					{
						IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(384, 396, 15, 15), questLogButtons[j].bounds.X, questLogButtons[j].bounds.Y, questLogButtons[j].bounds.Width, questLogButtons[j].bounds.Height, questLogButtons[j].containsPoint(Game1.getOldMouseX(), Game1.getOldMouseY()) ? Color.Wheat : Color.White, 4f, drawShadow: false);
						if (pages[currentPage][j].ShouldDisplayAsNew() || pages[currentPage][j].ShouldDisplayAsComplete())
						{
							Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(questLogButtons[j].bounds.X + 64 + 4, questLogButtons[j].bounds.Y + 44), new Rectangle(pages[currentPage][j].ShouldDisplayAsComplete() ? 341 : 317, 410, 23, 9), Color.White, 0f, new Vector2(11f, 4f), 4f + Game1.dialogueButtonScale * 10f / 250f, flipped: false, 0.99f);
						}
						else
						{
							Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(questLogButtons[j].bounds.X + 32, questLogButtons[j].bounds.Y + 28), pages[currentPage][j].IsTimedQuest() ? new Rectangle(410, 501, 9, 9) : new Rectangle(395 + (pages[currentPage][j].IsTimedQuest() ? 3 : 0), 497, 3, 8), Color.White, 0f, Vector2.Zero, 4f, flipped: false, 0.99f);
						}
						pages[currentPage][j].IsTimedQuest();
						SpriteText.drawString(b, pages[currentPage][j].GetName(), questLogButtons[j].bounds.X + 128 + 4, questLogButtons[j].bounds.Y + 20);
					}
				}
			}
			else
			{
				...
				{
					for (int i = 0; i < _objectiveText.Count; i++)
					{
						if (_shownQuest != null)
						{
							_ = _shownQuest is SpecialOrder;
						}
						string parsed_text = Game1.parseText(_objectiveText[i], width: base.width - 192, whichFont: Game1.dialogueFont);
						bool display_as_complete = false;
						if (_shownQuest != null && _shownQuest is SpecialOrder)
						{
							display_as_complete = (_shownQuest as SpecialOrder).objectives[i].IsComplete();
						}
						...
						if (_shownQuest != null && _shownQuest is SpecialOrder)
						{
							OrderObjective order_objective = (_shownQuest as SpecialOrder).objectives[i];
