...
namespace StardewValley.Menus
{
	...
	public abstract class IClickableMenu
	{
			...
			foreach (FieldInfo f in fields)
			{
				...
				else if (f.FieldType == typeof(List<ClickableComponent>))
				{
					...
					for (int l = l4.Count - 1; l >= 0; l--)
					{
						if (l4[l] != null)
						{
							allClickableComponents.Add(l4[l]);
						}
					}
				}
				else if (f.FieldType == typeof(List<ClickableTextureComponent>))
				{
					...
					for (int k = l3.Count - 1; k >= 0; k--)
					{
						if (l3[k] != null)
						{
							allClickableComponents.Add(l3[k]);
						}
					}
				}
				else if (f.FieldType == typeof(List<ClickableAnimatedComponent>))
				{
					List<ClickableAnimatedComponent> l2 = (List<ClickableAnimatedComponent>)f.GetValue(this);
					for (int j = l2.Count - 1; j >= 0; j--)
					{
						if (l2[j] != null)
						{
							allClickableComponents.Add(l2[j]);
						}
					}
				}
				else if (f.FieldType == typeof(List<Bundle>))
				{
					List<Bundle> n = (List<Bundle>)f.GetValue(this);
					for (int i = n.Count - 1; i >= 0; i--)
					{
						if (n[i] != null)
						{
							allClickableComponents.Add(n[i]);
						}
					}
				}
				...
				else if (f.FieldType == typeof(Dictionary<int, List<List<ClickableTextureComponent>>>))
				{
					...
					{
						foreach (List<ClickableTextureComponent> l5 in value)
						{
							allClickableComponents.AddRange(l5);
						}
					}
				}
				else
				{
					...
					foreach (ClickableTextureComponent m in ((Dictionary<int, ClickableTextureComponent>)f.GetValue(this)).Values)
					{
						allClickableComponents.Add(m);
					}
				}
			}
			GameMenu game_menu;
		...
		public static void drawToolTip(SpriteBatch b, string hoverText, string hoverTitle, Item hoveredItem, bool heldItem = false, int healAmountToDisplay = -1, int currencySymbol = 0, int extraItemToShowIndex = -1, int extraItemToShowAmount = -1, CraftingRecipe craftingIngredients = null, int moneyAmountToShowAtBottom = -1)
		{
			...
			int width = Math.Max((healAmountToDisplay != -1) ? ((int)Game1.smallFont.MeasureString(healAmountToDisplay + "+ Energy").X + 32) : 0, Math.Max((int)Game1.smallFont.MeasureString(hoverText).X, (hoverTitle != null) ? ((int)Game1.dialogueFont.MeasureString(hoverTitle).X) : 0)) + 32;
			if (edibleItem)
			{
				int maxStat = 9999;
				int buffer = 92;
				width = (int)Math.Max(width, Math.Max(Game1.smallFont.MeasureString(Game1.content.LoadString("Strings\\UI:ItemHover_Energy", maxStat)).X + (float)buffer, Game1.smallFont.MeasureString(Game1.content.LoadString("Strings\\UI:ItemHover_Health", maxStat)).X + (float)buffer));
			}
		...
		public static void drawHoverText(SpriteBatch b, StringBuilder text, SpriteFont font, int xOffset = 0, int yOffset = 0, int moneyAmountToDisplayAtBottom = -1, string boldTitleText = null, int healAmountToDisplay = -1, string[] buffIconsToDisplay = null, Item hoveredItem = null, int currencySymbol = 0, int extraItemToShowIndex = -1, int extraItemToShowAmount = -1, int overrideX = -1, int overrideY = -1, float alpha = 1f, CraftingRecipe craftingIngredients = null, IList<Item> additional_craft_materials = null)
		{
			...
			drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256, 60, 60), x, y, width + ((craftingIngredients != null) ? 21 : 0), height, Color.White * alpha);
			if (boldTitleText != null)
			{
				Vector2 bold_text_size = Game1.dialogueFont.MeasureString(boldTitleText);
				drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256, 60, 60), x, y, width + ((craftingIngredients != null) ? 21 : 0), (int)Game1.dialogueFont.MeasureString(boldTitleText).Y + 32 + (int)((hoveredItem != null && categoryName.Length > 0) ? font.MeasureString("asd").Y : 0f) - 4, Color.White * alpha, 1f, drawShadow: false);
			...
			if (hoveredItem != null && categoryName.Length > 0)
			{
				...
				if (hoveredItem is Tool && (hoveredItem as Tool).GetTotalForgeLevels() > 0)
				{
					...
					{
						Utility.drawTextWithShadow(b, " (" + forges + "/" + (hoveredItem as Tool).GetMaxForges() + ")", font, new Vector2((float)(x + 16) + font.MeasureString(forged_string2).X, y + 16 + 4), Color.DimGray, 1f, -1f, 2, 2);
					}
			...
			{
				b.DrawString(font, text, new Vector2(x + 16, y + 16 + 4) + new Vector2(2f, 2f), Game1.textShadowColor * alpha);
				b.DrawString(font, text, new Vector2(x + 16, y + 16 + 4) + new Vector2(0f, 2f), Game1.textShadowColor * alpha);
				b.DrawString(font, text, new Vector2(x + 16, y + 16 + 4) + new Vector2(2f, 0f), Game1.textShadowColor * alpha);
				b.DrawString(font, text, new Vector2(x + 16, y + 16 + 4), Game1.textColor * 0.9f * alpha);
			...
			if (healAmountToDisplay != -1)
			{
				...
				else if (stamina_recovery != -300)
				{
					...
					Utility.drawTextWithShadow(b, Game1.content.LoadString("Strings\\UI:ItemHover_Energy", string.Concat(stamina_recovery)), font, new Vector2(x + 16 + 34 + 4, y + 16), Game1.textColor);
					y += 34;
				}
			}
			if (buffIconsToDisplay != null)
			...
			if (moneyAmountToDisplayAtBottom > -1)
			{
				b.DrawString(font, string.Concat(moneyAmountToDisplayAtBottom), new Vector2(x + 16, y + 16 + 4) + new Vector2(2f, 2f), Game1.textShadowColor);
				b.DrawString(font, string.Concat(moneyAmountToDisplayAtBottom), new Vector2(x + 16, y + 16 + 4) + new Vector2(0f, 2f), Game1.textShadowColor);
				b.DrawString(font, string.Concat(moneyAmountToDisplayAtBottom), new Vector2(x + 16, y + 16 + 4) + new Vector2(2f, 0f), Game1.textShadowColor);
				b.DrawString(font, string.Concat(moneyAmountToDisplayAtBottom), new Vector2(x + 16, y + 16 + 4), Game1.textColor);
				switch (currencySymbol)
				{
				case 0:
					b.Draw(Game1.debrisSpriteSheet, new Vector2((float)(x + 16) + font.MeasureString(string.Concat(moneyAmountToDisplayAtBottom)).X + 20f, y + 16 + 16), Game1.getSourceRectForStandardTileSheet(Game1.debrisSpriteSheet, 8, 16, 16), Color.White, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, 0.95f);
					break;
				case 1:
					b.Draw(Game1.mouseCursors, new Vector2((float)(x + 8) + font.MeasureString(string.Concat(moneyAmountToDisplayAtBottom)).X + 20f, y + 16 - 5), new Rectangle(338, 400, 8, 8), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
					break;
				case 2:
					b.Draw(Game1.mouseCursors, new Vector2((float)(x + 8) + font.MeasureString(string.Concat(moneyAmountToDisplayAtBottom)).X + 20f, y + 16 - 7), new Rectangle(211, 373, 9, 10), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
					break;
				case 4:
					b.Draw(Game1.objectSpriteSheet, new Vector2((float)(x + 8) + font.MeasureString(string.Concat(moneyAmountToDisplayAtBottom)).X + 20f, y + 16 - 7), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, 858, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
					break;
				}
				y += 48;
			}
			if (extraItemToShowIndex != -1)
			{
				...
				y += 20;
				b.DrawString(font, requirement, new Vector2(x + 16, y + 4) + new Vector2(2f, 2f), Game1.textShadowColor);
				b.DrawString(font, requirement, new Vector2(x + 16, y + 4) + new Vector2(0f, 2f), Game1.textShadowColor);
				b.DrawString(font, requirement, new Vector2(x + 16, y + 4) + new Vector2(2f, 0f), Game1.textShadowColor);
				b.DrawString(Game1.smallFont, requirement, new Vector2(x + 16, y + 4), Game1.textColor);
				b.Draw(Game1.objectSpriteSheet, new Vector2(x + 16 + (int)font.MeasureString(requirement).X + 21, y), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, extraItemToShowIndex, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
			}
