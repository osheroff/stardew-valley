...
namespace StardewValley.Menus
{
	public class ProfileMenu : IClickableMenu
	{
		...
		public NPC GetTemporaryCharacter(Character character)
		{
			...
			return new NPC(new AnimatedSprite("Characters\\" + (character as NPC).getTextureName(), 0, 16, height / 4), new Vector2(0f, 0f), character.Name, character.facingDirection, character.name, null, portrait, eventActor: true);
		}
		...
		{
			int index = _charactersList.IndexOf(_target);
			if (index == -1)
			{
			...
			for (index += offset; index < 0; index += _charactersList.Count)
			{
			}
			while (index >= _charactersList.Count)
			{
				index -= _charactersList.Count;
			}
			_SetCharacter(_charactersList[index]);
