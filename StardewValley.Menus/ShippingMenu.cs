		...
		public override void draw(SpriteBatch b)
		{
				...
				foreach (ClickableTextureComponent c in categories)
				{
					if (introTimer < 2500 - j * 500)
					{
						...
						if (c.visible)
						{
							...
							categoryItems[j][0].drawInMenu(b, start + new Vector2(-88f, yOffset + 16), 1f, 1f, 0.9f, StackDrawType.Hide);
						}
						...
						for (int l = 0; l < 6; l++)
						{
							b.Draw(Game1.mouseCursors, start + new Vector2(-itemSlotWidth - 192 - 24 + l * 6 * 4, 12f), new Rectangle(355, 476, 7, 11), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.88f);
						}
						categoryDials[j].draw(b, start + new Vector2(-itemSlotWidth - 192 - 48 + 4, 20f), categoryTotals[j]);
			...
			else
			{
				...
				for (int i = currentTab * itemsPerCategoryPage; i < currentTab * itemsPerCategoryPage + itemsPerCategoryPage; i++)
				{
					...
					{
						string line_string = "";
						line_string = ((!show_single_item_price) ? (item.DisplayName + ((item.Stack > 1) ? (" x" + item.Stack) : "")) : (item.DisplayName + " x" + Game1.content.LoadString("Strings\\StringsFromCSFiles:LoadGameMenu.cs.11020", singleItemValues[item])));
						SpriteText.drawString(b, line_string, (int)position.X + 64 + 12, (int)position.Y + 12);
						string dots = ".";
						for (int k = 0; k < width - 96 - SpriteText.getWidthOfString(line_string + Game1.content.LoadString("Strings\\StringsFromCSFiles:LoadGameMenu.cs.11020", itemValues[item])); k += SpriteText.getWidthOfString(" ."))
						{
							dots += " .";
						}
						SpriteText.drawString(b, dots, (int)position.X + 80 + SpriteText.getWidthOfString(line_string), (int)position.Y + 8);
					...
					{
						string dotsAndName = "";
						dotsAndName = ((!show_single_item_price) ? (item.DisplayName + ((item.Stack > 1) ? (" x" + item.Stack) : ".")) : (item.DisplayName + " x" + Game1.content.LoadString("Strings\\StringsFromCSFiles:LoadGameMenu.cs.11020", singleItemValues[item])));
