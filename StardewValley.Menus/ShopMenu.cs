		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			if (heldItem == null && !readOnly)
			{
				...
				if (toSell != null)
				{
					...
					{
						chargePlayer(Game1.player, currency, -((int)((toSell is Object) ? ((float)(toSell as Object).sellToStorePrice(-1L) * sellPercentage) : ((float)(toSell.salePrice() / 2) * sellPercentage)) * toSell.Stack));
			...
			for (int i = 0; i < forSaleButtons.Count; i++)
			{
				...
				{
					int toBuy = ((!Game1.oldKBState.IsKeyDown(Keys.LeftShift)) ? 1 : Math.Min(Math.Min(5, getPlayerCurrencyAmount(Game1.player, currency) / Math.Max(1, itemPriceAndStock[forSale[index]][0])), Math.Max(1, itemPriceAndStock[forSale[index]][1])));
			...
			if (heldItem == null && !readOnly)
			{
				...
				if (toSell != null)
				{
					...
					else
					{
						...
						if (toSell is Object && (int)(toSell as Object).edibility != -300 && Game1.random.NextDouble() < 0.039999999105930328 && Game1.currentLocation is ShopLocation)
						{
							(Game1.currentLocation as ShopLocation).itemsToStartSellingTomorrow.Add((Item)toSell.GetSalableInstance());
						}
						if (inventory.getItemAt(x, y) == null)
						{
							Game1.playSound("sell");
