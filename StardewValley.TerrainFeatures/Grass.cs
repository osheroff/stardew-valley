	...
	public class Grass : TerrainFeature
	{
		...
		public override void loadSprite()
		{
			try
			{
				...
				if ((byte)grassType == 1)
				{
					switch (Game1.GetSeasonForLocation(currentLocation))
					{
					case "spring":
		...
		public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation, GameLocation location)
		{
			...
			if ((t != null && t is MeleeWeapon && (int)((MeleeWeapon)t).type != 2) || explosion > 0)
			{
				...
				int numberOfWeedsToDestroy = 0;
				numberOfWeedsToDestroy = ((explosion <= 0) ? 1 : Math.Max(1, explosion + 2 - Game1.recentMultiplayerRandom.Next(2)));
				...
				if ((int)numberOfWeeds <= 0)
				{
					if ((byte)grassType != 1)
					{
						Random grassRandom = (Game1.IsMultiplayer ? Game1.recentMultiplayerRandom : new Random((int)((float)Game1.uniqueIDForThisGame + tileLocation.X * 1000f + tileLocation.Y * 11f + (float)Game1.CurrentMineLevel + (float)Game1.player.timesReachedMineBottom)));
					...
					{
						Random obj = (Game1.IsMultiplayer ? Game1.recentMultiplayerRandom : new Random((int)((float)Game1.uniqueIDForThisGame + tileLocation.X * 1000f + tileLocation.Y * 11f)));
