	...
	public class FruitTree : TerrainFeature
	{
		...
		public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation, GameLocation location)
		{
			...
			else if ((int)growthStage >= 3)
			{
				...
				shake(tileLocation, doEvenIfStillShaking: true, location);
				float damage = 1f;
				Random debrisRandom = ((!Game1.IsMultiplayer) ? new Random((int)((float)Game1.uniqueIDForThisGame + tileLocation.X * 7f + tileLocation.Y * 11f + (float)Game1.stats.DaysPlayed + (float)health)) : Game1.recentMultiplayerRandom);
				if (explosion > 0)
				{
					damage = explosion;
				}
		...
		public override void drawInMenu(SpriteBatch spriteBatch, Vector2 positionOnScreen, Vector2 tileLocation, float scale, float layerDepth)
		{
			...
			if ((int)growthStage < 4)
			{
				...
				sourceRect = (int)growthStage switch
				{
		...
		public override void draw(SpriteBatch spriteBatch, Vector2 tileLocation)
		{
			...
			{
				spriteBatch.Draw(texture, Game1.GlobalToLocal(Game1.viewport, j.position), new Rectangle((24 + Utility.getSeasonNumber(season)) * 16, (int)treeType * 5 * 16, 8, 8), Color.White, j.rotation, Vector2.Zero, 4f, SpriteEffects.None, (float)getBoundingBox(tileLocation).Bottom / 10000f + 0.01f);
			}
		}
	}
}
