	...
	public class CosmeticPlant : Grass
	{
		...
		public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation, GameLocation location = null)
		{
			if ((t != null && t is MeleeWeapon && (int)((MeleeWeapon)t).type != 2) || explosion > 0)
			{
				...
				numberOfWeedsToDestroy = ((explosion <= 0) ? (((int)t.upgradeLevel == 3) ? 3 : ((int)t.upgradeLevel + 1)) : Math.Max(1, explosion + 2 - Game1.random.Next(2)));
				...
				{
					Random grassRandom = new Random((int)((float)Game1.uniqueIDForThisGame + tileLocation.X * 7f + tileLocation.Y * 11f + (float)Game1.CurrentMineLevel + (float)Game1.player.timesReachedMineBottom));
