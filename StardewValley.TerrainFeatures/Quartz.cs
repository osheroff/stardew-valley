		...
		public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation, GameLocation location = null)
		{
			if ((float)health > 0f)
			{
				...
				if (damage > 0f)
				{
					...
					if ((float)health <= 0f)
					{
						Random crystalRandom = new Random((int)((float)Game1.uniqueIDForThisGame + tileLocation.X * 7f + tileLocation.Y * 11f + (float)Game1.CurrentMineLevel + (float)Game1.player.timesReachedMineBottom));
