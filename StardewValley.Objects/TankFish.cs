		...
		public virtual void Draw(SpriteBatch b, float alpha, float draw_layer)
		{
			...
			else
			{
				for (int slice = 0; slice < 24 / slice_size; slice++)
				{
					...
					b.Draw(_texture, Game1.GlobalToLocal(GetWorldPosition() + new Vector2(draw_offset + slice * slice_offset, bob + (float)(Math.Sin((double)(slice * 20) + Game1.currentGameTime.TotalGameTime.TotalSeconds * 25.0 * (double)time_multiplier + (double)(position_multiplier * position.X / 16f)) * (double)wiggle_amount * (double)multiplier * (double)velocity_multiplier)) * 4f * scale), new Rectangle(sprite_sheet_x + slice * slice_size, sprite_sheet_y, slice_size, 24), Color.White * alpha, 0f, new Vector2(0f, 12f), 4f * scale, sprite_effects, draw_layer);
				}
			}
