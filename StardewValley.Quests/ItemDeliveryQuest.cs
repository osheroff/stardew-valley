	...
	public class ItemDeliveryQuest : Quest
	{
		...
		public List<NPC> GetValidTargetList()
		{
			...
			foreach (Farmer farmer in Game1.getAllFarmers())
			{
				...
				{
					if (valid_targets[j].Name.Equals(farmer.spouse))
					{
			...
			for (int i = 0; i < valid_targets.Count; i++)
			{
				if (!valid_targets[i].Name.Equals("Sandy"))
				{
					continue;
				}
		...
		public void loadQuestInfo()
		{
			...
			actualTarget = valid_targets[random.Next(valid_targets.Count)];
			...
			else
			{
				...
				if (Game1.objectInformation[item].Split('/')[3].Split(' ')[0].Equals("Cooking") && !target.Value.Equals("Wizard"))
				{
					...
					else
					{
						...
						questDescriptions = new DescriptionElement[5]
						{
							...
							new DescriptionElement("Strings\\StringsFromCSFiles:ItemDeliveryQuest.cs.13373", day, deliveryItem.Value, actualTarget)
						};
					...
					parts.Clear();
					int rand5 = random.Next(questDescriptions.Count());
				...
				else if (random.NextDouble() < 0.5 && Convert.ToInt32(Game1.objectInformation[item].Split('/')[2]) > 0)
				{
					...
					{
						parts.Clear();
						int rand4 = random.Next(questDescriptions.Count());
				...
				else
				{
					...
					{
						new DescriptionElement("Strings\\StringsFromCSFiles:ItemDeliveryQuest.cs.13480", actualTarget, deliveryItem.Value),
						...
						new DescriptionElement("Strings\\StringsFromCSFiles:ItemDeliveryQuest.cs.13518", actualTarget, deliveryItem.Value),
					...
					parts.Clear();
					int rand3 = random.Next(questDescriptions.Count());
