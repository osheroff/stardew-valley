	...
	public class DescriptionElement : INetObject<NetFields>
	{
		...
		public string loadDescriptionElement()
		{
			...
			for (int i = 0; i < temp.param.Count; i++)
			{
				...
				{
					Game1.objectInformation.TryGetValue((temp.param[i] as Object).parentSheetIndex, out var objectInformation);
			...
			{
			default:
				returnString = Game1.content.LoadString(temp.xmlKey);
				if (xmlKey.Contains("Dialogue.cs.7") || xmlKey.Contains("Dialogue.cs.8"))
				{
					returnString = Game1.content.LoadString(temp.xmlKey).Replace("/", " ");
					returnString = ((returnString[0] == ' ') ? returnString.Substring(1) : returnString);
				}
				break;
			case 1:
				returnString = Game1.content.LoadString(temp.xmlKey, temp.param[0]);
				break;
			case 2:
				returnString = Game1.content.LoadString(temp.xmlKey, temp.param[0], temp.param[1]);
				break;
			case 3:
				returnString = Game1.content.LoadString(temp.xmlKey, temp.param[0], temp.param[1], temp.param[2]);
				break;
			case 4:
				returnString = Game1.content.LoadString(temp.xmlKey, temp.param[0], temp.param[1], temp.param[2], temp.param[3]);
				break;
			}
			return returnString;
		}
