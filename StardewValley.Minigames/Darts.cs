		...
		public void draw(SpriteBatch b)
		{
			...
			else if (points >= 0)
			{
				...
				if (currentGameState == GameState.ShowScore || currentGameState == GameState.Scoring)
				{
					...
					string_to_draw = ((currentGameState != GameState.ShowScore) ? (" " + lastHitAmount + " ") : (" " + lastHitString + " "));
					SpriteText.drawStringWithScrollCenteredAt(b, string_to_draw, (int)(score_position.X + score_shake.X), (int)(score_position.Y + score_shake.Y), "", 1f, 1, 2);
				}
			}
