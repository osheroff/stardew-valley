	...
	public class AbigailGame : IMinigame
	{
							...
							if (monsters[m].takeDamage(bullets[j].damage))
							{
								...
								if (shootoutLevel)
								{
									if (whichWave == 12 && monsters[m].type == -2)
									{
										...
										for (int l = 0; l < 30; l++)
										{
											temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(512, 1696, 16, 16), 70f, 6, 0, new Vector2(monsters[m].position.X + Game1.random.Next(-TileSize, TileSize), monsters[m].position.Y + Game1.random.Next(-TileSize, TileSize)) + topLeftScreenCoordinate + new Vector2(TileSize / 2, TileSize / 2), flicker: false, flipped: false, 1f, 0f, Color.White, 3f, 0f, 0f, 0f, local: true)
											{
											...
											if (l % 4 == 0)
											{
												temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(464, 1792, 16, 16), 80f, 5, 0, new Vector2(monsters[m].position.X + Game1.random.Next(-TileSize, TileSize), monsters[m].position.Y + Game1.random.Next(-TileSize, TileSize)) + topLeftScreenCoordinate + new Vector2(TileSize / 2, TileSize / 2), flicker: false, flipped: false, 1f, 0f, Color.White, 3f, 0f, 0f, 0f, local: true)
												{
													delayBeforeAnimationStart = l * 75
												});
											}
											if (l % 3 == 0)
											{
												temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(544, 1728, 16, 16), 100f, 4, 0, new Vector2(monsters[m].position.X + Game1.random.Next(-TileSize, TileSize), monsters[m].position.Y + Game1.random.Next(-TileSize, TileSize)) + topLeftScreenCoordinate + new Vector2(TileSize / 2, TileSize / 2), flicker: false, flipped: false, 1f, 0f, Color.White, 3f, 0f, 0f, 0f, local: true)
												{
													delayBeforeAnimationStart = l * 75
												});
											}
		...
		public bool tick(GameTime time)
		{
			...
			if (motionPause <= 0)
			{
				...
				{
					activePowerups[activePowerups.ElementAt(i3).Key] -= time.ElapsedGameTime.Milliseconds;
					if (activePowerups[activePowerups.ElementAt(i3).Key] <= 0)
					{
						activePowerups.Remove(activePowerups.ElementAt(i3).Key);
					}
				}
			...
			case 8:
			{
				for (int l = 0; l < 16; l++)
				{
					for (int j4 = 0; j4 < 16; j4++)
					{
						if (newMap[l, j4] == 5)
						{
							newMap[l, j4] = ((!(Game1.random.NextDouble() < 0.5)) ? 1 : 0);
						}
					}
				}
				for (int k = 0; k < 16; k++)
				{
					newMap[k, 8] = ((Game1.random.NextDouble() < 0.5) ? 8 : 9);
				}
				newMap[8, 4] = 7;
				newMap[8, 12] = 7;
				newMap[9, 12] = 7;
			...
			else if (endCutscene)
			{
				...
				case 3:
				{
					for (int i = 0; i < 16; i++)
					{
						for (int n = 0; n < 16; n++)
						{
							b.Draw(Game1.mouseCursors, topLeftScreenCoordinate + new Vector2(i, n) * 16f * 3f + new Vector2(0f, newMapPosition - 16 * TileSize), new Rectangle(464 + 16 * map[i, n] + ((map[i, n] == 5 && cactusDanceTimer > 800f) ? 16 : 0), 1680 - world * 16, 16, 16), Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 0f);
						}
					}
