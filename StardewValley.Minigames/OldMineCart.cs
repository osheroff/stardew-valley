...
namespace StardewValley.Minigames
{
	public class OldMineCart : IMinigame
	{
		...
		public void draw(SpriteBatch b)
		{
			...
			for (int m = 0; m < lakeDecor.Count; m++)
			{
				b.Draw(texture, new Vector2((float)(m * tileSize) - lakeSpeedAccumulator, lakeDecor.ElementAt(m).Y * tileSize), new Rectangle(32 + lakeDecor.ElementAt(m).X * tileSize, 0, 16, 16), (lakeDecor.ElementAt(m).X == 0) ? midBGTint : lakeTint);
			}
			for (int i4 = 0; i4 <= screenWidth / 96 + 2; i4++)
			{
				b.Draw(texture, new Vector2(0f - backBGPosition + (float)(i4 * 96), tileSize * 2), new Rectangle(64, 162, 96, 111), backBGTint);
			}
			for (int i3 = 0; i3 < screenWidth / 96 + 2; i3++)
			{
				b.Draw(texture, new Vector2(0f - midBGPosition + (float)(i3 * 96), 0f), new Rectangle(64, 0, 96, 162), midBGTint);
			}
			for (int i2 = 0; i2 < track.Count; i2++)
			{
				if (track.ElementAt(i2).X != 0)
				{
					b.Draw(texture, new Vector2(0f - speedAccumulator + (float)(i2 * tileSize), (track.ElementAt(i2).Y + ytileOffset) * tileSize - tileSize), new Rectangle(160 + (track.ElementAt(i2).X - 1) * 16, 144, 16, 32), trackTint);
					float darkness = 0f;
					for (int j2 = track.ElementAt(i2).Y + 1; j2 < screenHeight / tileSize; j2++)
					{
						b.Draw(texture, new Vector2(0f - speedAccumulator + (float)(i2 * tileSize), (j2 + ytileOffset) * tileSize), new Rectangle(16 + ((j2 % 2 == 0) ? ((track.ElementAt(i2).X + 1) % 2) : (track.ElementAt(i2).X % 2)) * 16, 32, 16, 16), trackTint);
						b.Draw(texture, new Vector2(0f - speedAccumulator + (float)(i2 * tileSize), (j2 + ytileOffset) * tileSize), new Rectangle(16 + ((j2 % 2 == 0) ? ((track.ElementAt(i2).X + 1) % 2) : (track.ElementAt(i2).X % 2)) * 16, 32, 16, 16), trackShadowTint * darkness);
						darkness += 0.1f;
					}
				}
			}
			...
			{
				switch (obstacles[n].X)
				{
				case 1:
					b.Draw(texture, new Vector2(0f - speedAccumulator + (float)(n * tileSize), (obstacles[n].Y + ytileOffset) * tileSize), new Rectangle(16, 0, 16, 16), Color.White);
					break;
				case 2:
					b.Draw(Game1.debrisSpriteSheet, new Vector2(0f - speedAccumulator + (float)(n * tileSize), (obstacles[n].Y + ytileOffset) * tileSize), Game1.getSourceRectForStandardTileSheet(Game1.debrisSpriteSheet, 8), Color.White, 0f, new Vector2(32f, 0f), 0.25f, SpriteEffects.None, 0f);
					break;
				}
			}
			...
			for (int j3 = 0; j3 < waterfallWidth; j3 += 2)
			{
				for (int j = -2; j <= screenHeight / tileSize + 1; j++)
				{
					b.Draw(texture, new Vector2((float)(screenWidth + tileSize * j3) - waterFallPosition, (float)(j * tileSize) + lakeSpeedAccumulator * 2f), new Rectangle(48, 32, 16, 16), waterfallTint);
				}
			}
			if (gameMode != 2 && totalMotion < distanceToTravel + screenWidth / tileSize)
			{
				if (gameMode != 4)
				{
					...
					{
						b.Draw(texture, new Vector2(screenWidth - l * (tileSize + 2) - tileSize, 0f), new Rectangle(0, 0, 16, 16), Color.White);
					}
				}
				...
				for (int k = 0; k < 4; k++)
				{
					b.Draw(Game1.staminaRect, new Rectangle(pixelScale + tileSize * 8, pixelScale + k * (pixelScale / 4), pixelScale / 4, pixelScale / 4), (k % 2 == 0) ? Color.White : Color.Black);
					b.Draw(Game1.staminaRect, new Rectangle(pixelScale + tileSize * 8 + pixelScale / 4, pixelScale + k * (pixelScale / 4), pixelScale / 4, pixelScale / 4), (k % 2 == 0) ? Color.Black : Color.White);
				}
