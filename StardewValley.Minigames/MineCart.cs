	...
	public class MineCart : IMinigame
	{
			...
			protected override void _GenerateTrack()
			{
				...
				for (int j = 0; j < 4; j++)
				{
					...
					if (j != 1)
					{
						...
						{
							target_y += Utility.GetRandom(staggerPattern);
						}
					}
					...
					if (j == 0 || j == 2)
					{
						...
						AddTrack(track);
						current_balance_tracks.Add(track as BalanceTrack);
						_game.generatorPosition.X++;
		...
		public bool tick(GameTime time)
		{
			...
			if (gameState == GameStates.Ingame)
			{
				...
				{
					int k;
					for (k = 0; k < checkpointPositions.Count; k++)
					{
						GetTrackForXPosition(checkpointPositions[k] * tileSize);
						if (!(player.position.X >= (float)(checkpointPositions[k] * tileSize)))
						{
							break;
						}
						foreach (Entity entity2 in _entities)
						{
							if (entity2 is CheckpointIndicator && (int)(entity2.position.X / (float)tileSize) == checkpointPositions[k])
							{
			...
			float parallax_scroll_speed = 0f;
			parallax_scroll_speed = num / (float)tileSize;
		...
		public void draw(SpriteBatch b)
		{
			...
			if (gameState != GameStates.Map)
			{
				...
				{
					for (int i5 = 0; i5 <= screenWidth / tileSize + 1; i5++)
					{
						b.Draw(texture, TransformDraw(new Rectangle(i5 * tileSize - (int)lakeSpeedAccumulator % tileSize, tileSize * 9, tileSize, screenHeight - 96)), lakeBGSource, lakeTint, 0f, Vector2.Zero, SpriteEffects.None, 0.9f);
					}
					for (int i4 = 0; i4 < lakeDecor.Count; i4++)
					{
						lakeDecor[i4].Draw(b);
					}
					...
					for (int i2 = 0; i2 < screenWidth / midBGSource.Width + 2; i2++)
					{
						b.Draw(texture, TransformDraw(new Vector2(0f - midBGPosition + (float)(i2 * midBGSource.Width), 162 - midBGSource.Height + midBGYOffset)), midBGSource, midBGTint, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.6f);
					}
				}
			}
			foreach (Entity entity in _entities)
			...
			if (gameState == GameStates.Title)
			{
				...
				if (gameMode == 2)
				{
					...
					for (int n = 0; n < 5 && n < _currentHighScores.Count; n++)
					{
						...
						KeyValuePair<string, int> score = _currentHighScores[n];
						int score_text_width = (int)Game1.dialogueFont.MeasureString(string.Concat(score.Value)).X / 4;
						b.DrawString(Game1.dialogueFont, "#" + (n + 1), TransformDraw(draw_position), color, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.199f);
						...
						b.DrawString(Game1.dialogueFont, "#" + (n + 1), TransformDraw(draw_position + shadow_offset), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1999f);
