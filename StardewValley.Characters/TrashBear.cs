	...
	public class TrashBear : NPC
	{
		...
		private void updateItemWanted()
		{
			...
			foreach (string v in CraftingRecipe.cookingRecipes.Values)
			{
				if (counter == position)
				{
					string craft_result = v.Split('/')[2];
					craft_result = craft_result.Split(' ')[0];
					itemWantedIndex = Convert.ToInt32(craft_result);
					break;
				}
				counter++;
			}
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (showWantBubbleTimer > 0)
			{
				...
				yOffset -= 40f;
				b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(mailbox_position.X * 64 + 32, (float)(mailbox_position.Y * 64 - 96 - 48) + yOffset)), new Rectangle(141, 465, 20, 24), Color.White * 0.75f, 0f, Vector2.Zero, 4f, SpriteEffects.None, draw_layer + 1E-06f);
				b.Draw(Game1.objectSpriteSheet, Game1.GlobalToLocal(Game1.viewport, new Vector2(mailbox_position.X * 64 + 64 + 8, (float)(mailbox_position.Y * 64 - 64 - 32 - 8) + yOffset)), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, itemWantedIndex, 16, 16), Color.White, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, draw_layer + 1E-05f);
			}
		}
	}
}
