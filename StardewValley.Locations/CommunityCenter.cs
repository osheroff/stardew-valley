	...
	public class CommunityCenter : GameLocation
	{
		...
		private void initAreaBundleConversions()
		{
			...
			for (int i = 0; i < 7; i++)
			{
				...
				bundleMutexes.Add(j);
				base.NetFields.AddField(j.NetFields);
			}
			...
			if (restoreAreaTimer > 0)
			{
				...
				case 1:
					if (Game1.IsMasterGame && Game1.random.NextDouble() < 0.4)
					{
						...
						Junimo j = new Junimo(v * 64f, restoreAreaIndex, temporary: true);
				...
				case 2:
					...
					if (Game1.IsMasterGame)
					{
						for (int i = characters.Count - 1; i >= 0; i--)
						{
							if (characters[i] is Junimo && (bool)(characters[i] as Junimo).temporaryJunimo)
							{
								characters.RemoveAt(i);
							}
						}
