...
namespace StardewValley.Locations
{
	public class IslandSouth : IslandLocation
	{
		public class IslandActivityAssigments
		{
			...
			public bool TryAssignment(Character character, List<Point> points, string dialogue_key, string animation_name = null, bool animation_required = false, double chance = 1.0, Dictionary<Character, string> last_activity_assignments = null)
			{
				...
				if (points.Count > 0 && (random.NextDouble() < chance || chance >= 1.0))
				{
					...
					assignment_string = ((!(animation_name == "") && animation_name != null) ? (current_point.X + " " + current_point.Y + " " + animation_name) : (current_point.X + " " + current_point.Y + " 2"));
		...
		public override bool performAction(string action, Farmer who, Location tileLocation)
		{
			...
			{
				string key = "";
				key = (resortOpenToday.Value ? (shouldToggleResort.Value ? "Strings\\Locations:IslandSouth_ResortOpenWillCloseSign" : "Strings\\Locations:IslandSouth_ResortOpenSign") : (shouldToggleResort.Value ? "Strings\\Locations:IslandSouth_ResortClosedWillOpenSign" : "Strings\\Locations:IslandSouth_ResortClosedSign"));
		...
		public override string GetLocationOverrideDialogue(NPC character)
		{
			...
			{
				dialogue_key = "Characters\\Dialogue\\" + character.Name + ":Resort_Entering";
				if (Game1.content.LoadStringReturnNullIfNotFound(dialogue_key) != null)
				{
					return dialogue_key;
				}
			...
			{
				dialogue_key = "Characters\\Dialogue\\" + character.Name + ":Resort_Leaving";
				if (Game1.content.LoadStringReturnNullIfNotFound(dialogue_key) != null)
				{
					return dialogue_key;
				}
