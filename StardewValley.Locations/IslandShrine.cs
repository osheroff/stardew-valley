...
namespace StardewValley.Locations
{
	public class IslandShrine : IslandForestLocation
	{
		...
		public virtual void AddMissingPedestals()
		{
			...
			position.X = 21f;
			position.Y = 27f;
			IslandGemBird.GemBirdType bird_type = IslandGemBird.GemBirdType.Amethyst;
			Object existing_pedestal = getObjectAtTile((int)position.X, (int)position.Y);
			bird_type = IslandGemBird.GetBirdTypeForLocation("IslandWest");
			if (existing_pedestal == null)
			{
				westPedestal = new ItemPedestal(position, null, lock_on_success: false, Color.White);
				objects.Add(position, westPedestal);
				westPedestal.requiredItem.Value = new Object(Vector2.Zero, IslandGemBird.GetItemIndex(bird_type), 1);
				westPedestal.successColor.Value = new Color(0, 0, 0, 0);
			}
			else if (existing_pedestal is ItemPedestal)
			{
				ItemPedestal pedestal4 = existing_pedestal as ItemPedestal;
				int item_index4 = IslandGemBird.GetItemIndex(bird_type);
			...
			position.X = 27f;
			position.Y = 27f;
			existing_pedestal = getObjectAtTile((int)position.X, (int)position.Y);
			bird_type = IslandGemBird.GetBirdTypeForLocation("IslandEast");
			if (existing_pedestal == null)
			{
				eastPedestal = new ItemPedestal(position, null, lock_on_success: false, Color.White);
				objects.Add(position, eastPedestal);
				eastPedestal.requiredItem.Value = new Object(Vector2.Zero, IslandGemBird.GetItemIndex(bird_type), 1);
				eastPedestal.successColor.Value = new Color(0, 0, 0, 0);
			}
			else if (existing_pedestal is ItemPedestal)
			{
				ItemPedestal pedestal3 = existing_pedestal as ItemPedestal;
				int item_index3 = IslandGemBird.GetItemIndex(bird_type);
			...
			position.Y = 28f;
			existing_pedestal = getObjectAtTile((int)position.X, (int)position.Y);
			bird_type = IslandGemBird.GetBirdTypeForLocation("IslandSouth");
			if (existing_pedestal == null)
			{
				southPedestal = new ItemPedestal(position, null, lock_on_success: false, Color.White);
				objects.Add(position, southPedestal);
				southPedestal.requiredItem.Value = new Object(Vector2.Zero, IslandGemBird.GetItemIndex(bird_type), 1);
				southPedestal.successColor.Value = new Color(0, 0, 0, 0);
			}
			else if (existing_pedestal is ItemPedestal)
			{
				ItemPedestal pedestal2 = existing_pedestal as ItemPedestal;
				int item_index2 = IslandGemBird.GetItemIndex(bird_type);
			...
			position.Y = 25f;
			existing_pedestal = getObjectAtTile((int)position.X, (int)position.Y);
			bird_type = IslandGemBird.GetBirdTypeForLocation("IslandNorth");
			if (existing_pedestal == null)
			{
				northPedestal = new ItemPedestal(position, null, lock_on_success: false, Color.White);
				objects.Add(position, northPedestal);
				northPedestal.requiredItem.Value = new Object(Vector2.Zero, IslandGemBird.GetItemIndex(bird_type), 1);
				northPedestal.successColor.Value = new Color(0, 0, 0, 0);
			}
			else
			{
				if (!(existing_pedestal is ItemPedestal))
				{
					return;
				}
				ItemPedestal pedestal = existing_pedestal as ItemPedestal;
				int item_index = IslandGemBird.GetItemIndex(bird_type);
